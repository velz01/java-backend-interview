...
## 1. Поясните: Основные компоненты Kafka?

<div class="answer-block">

Кратко: Apache Kafka - это распределённая система обработки потоков, состоящая из нескольких ключевых компонентов
...
## 2. Поясните: Гарантии доставки в Kafka
...
## 3. Поясните: Распределение сообщений. Как сообщения распределяются по партициям?

<div class="answer-block">

Кратко: Продюсер определяет, в какой раздел отправить сообщение, используя следующие подходы:
...
## 4. Поясните, что такое partition в Kafka?
...
## 5. Поясните: Kafka. Consumer Group

<div class="answer-block">

Кратко: Consumer Group - это механизм для группировки консьюмеров, работающих с одним топиком, для:
...
## 6. Поясните: Разница между Kafka и RabbitMQ
...
## 7. Поясните, что такое топик в Kafka?
...
## 8. Поясните: Брокеры сообщений vs Rest
...
## 9. Каким образом гарантировать в Kafka идемпотентность, чтобы не было задвоений и потерь сообщений?
...
## 10. Каким образом в Kafka гарантируется последовательность сообщений?

<div class="answer-block">

Кратко: Сообщения в одной партиции записываются последовательно. Kafka гарантирует, что потребитель получит их в том же порядке, в котором они были записаны.
...
## 11. Поясните: Плюсы и минусы брокеров сообщений
...
## 12. Поясните: Смещение (offset) в Kafka
...
## 14. Поясните, что такое Producer и что такое Consumer?
...
## 15. Поясните: Pull и push модели, в чём разница?
...
## 16. Поясните: Из чего состоит Kafka кластер?

<div class="answer-block">

Кратко: Kafka-кластер — это распределенная система для передачи, хранения и обработки сообщений. Его основные компоненты:
...
## 17. Поясните: Повторное чтение данных из Kafka
...
## 18. Поясните: Acknowledgment в Kafka

<div class="answer-block">

Кратко: Acknowledgment (acks) - это механизм подтверждения записи сообщения в Kafka. Производитель получает подтверждение от брокера после записи сообщения
...
## 19. Каким образом работает Round Robin?
...
## 20. Какие именно существуют паттерны работы с очередями?
...
## 21. Поясните: Что сделать, чтобы сообщения попадали в одну партицию?
...
Кратко: За хранение ваших данных отвечает брокер (broker). Все данные хранятся в бинарном виде, и брокер мало знает про то, что они из себя представляют, и какова их структура.
...
## 30. Поясните: Архитектура брокера
...
## 31. Поясните: Архитектура консюмера
...
## 32. Поясните: Архитектура продюсера
...
## 33. Поясните: Архитектура топика
...
## 34. Поясните, в чём разница между Kafka Consumer и Kafka Stream?
...
## 35. Поясните, в чём разница между Kafka Streams и Apache Flink?
...
## 36. Поясните, в чём разница между Kafka и Flume?
...
## 37. Поясните, в чём разница между Kafka и RabbitMQ?
...
## 38. Зачем нужен Broker log cleaner thread?
...
## 39. Зачем нужен Consumer heartbeat thread?

<div class="answer-block">

Кратко: Consumer Heartbeat Thread отвечает за отправку периодических сигналов брокеру Kafka (в частности, координатору группы).
...
## 40. Зачем нужен Kafka Mirror Maker?

<div class="answer-block">

Кратко: Это инструмент, позволяющий реплицировать данные между кластерами Kafka, потенциально находящихся в разных дата-центрах.
...
## 42. Зачем нужен идемпотентный продюсер?

<div class="answer-block">

Кратко: Идемпотентный продюсер гарантирует exactly-once гарантию доставки, предотвращая дублирование записей в Kafka в случае 
...
## 43. Зачем нужен интерфейс Partitioner?

<div class="answer-block">

Кратко: Интерфейс Partitioner в Producer API определяет в какую партицию топика будет отправлено сообщение. Partitioner по-умолчанию
...
## 44. Зачем нужен координатор группы?

<div class="answer-block">

Кратко: Координатор группы отвечает за управление группами потребителей. Он управляет членством в группах потребителей, назначает
...
## 45. Зачем нужен метод position()?

<div class="answer-block">

Кратко: Метод position() возвращает смещение следующей записи, которая будет извлечена для данной партиции. Это полезно для
...
## 46. Зачем нужна Schema Registry?

<div class="answer-block">

Кратко: Kafka Schema Registry предоставляет RESTful интерфейс для хранения и извлечения схем Avro. Schema Registry используется
...
## 47. Зачем нужны методы commitSync() и commitAsync()?
...
## 48. Зачем нужны методы subscribe() и poll()?
...
## 49. Каким образом Kafka обеспечивает версионирование сообщений?
...
## 50. Каким образом Kafka обрабатывает задержку консюмера?

<div class="answer-block">

Кратко: Задержка (лаг) консюмера в Kafka относится к разнице между оффсетом последнего созданного сообщения и оффсетом последнего
...
## 51. Каким образом Kafka обрабатывает сообщения?
...
## 52. Каким образом потребители получают сообщения от брокера?
...
## 53. Поясните: Какова роль AdminClient API?

<div class="answer-block">

Кратко: AdminClient API предоставляет операции для управления топиками, брокерами, конфигурацией и другими объектами Kafka. 
...
## 54. Поясните: Какова роль Connector API?

<div class="answer-block">

Кратко: Connector API в Apache Kafka является частью Kafka Connect, которая представляет собой инфраструктуру для интеграции
...
## 55. Поясните: Какова роль Consumer API?
...
## 56. Поясните: Какова роль Producer API?

<div class="answer-block">

Кратко: Используется для публикации потока сообщений в топики Kafka. Он управляет партицированием сообщений, сжатием и балансировкой 
...
## 58. Поясните: Какова роль Streams API?

<div class="answer-block">

Кратко: Это компонент Apache Kafka, предназначенный для создания приложений и микросервисов, которые обрабатывают потоки данных 
...
## 59. Поясните: Какова роль Transactions API?
...
## 60. Поясните: Настройки брокера Kafka

<div class="answer-block">

Кратко: ### Репликация и консистентность
...
## 61. Поясните: Настройки консюмера
...
## 62. Поясните: Настройки продюсера
...
## 63. Поясните: Настройки топика Kafka
...
## 64. Поясните: Основные API Kafka
...
## 66. Поясните: Преимущества очередей

<div class="answer-block">

Кратко: Основные преимущества использования очередей сообщений:
...
## 67. Поясните: Пример конфигурации Kafka Consumer
...
## 68. Поясните: Пример конфигурации Kafka Producer

<div class="answer-block">

```java
Кратко: import org.apache.kafka.clients.producer.KafkaProducer;
...
## 69. Поясните, что такое Apache Kafka?

<div class="answer-block">

Кратко: Это распределённая система с открытым исходным кодом, разработанная для высокоскоростной передачи больших объёмов данных 
