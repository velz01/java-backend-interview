...
- **Статья:** [Глубокое погружение в архитектуру Kafka](https://habr.com/ru/articles/972788/)
- **Статья:** [Kafka: consumer group, partition и offset](https://habr.com/ru/articles/466585/)
- **Статья:** [А вы Kafka пробовали: партиции, consumer groups, rebalance](https://habr.com/ru/articles/537846/)

---

**• Низкий приоритет**

## 1. Поясните, что такое Apache Kafka?
...
## 2. Поясните: Основные компоненты Kafka?
...
## 3. Поясните: Из чего состоит Kafka кластер?
...
## 5. Поясните, что такое топик в Kafka?

Топик в Kafka - это логический канал передачи сообщений. Состоит из разделов (партиций), которые представляют из себя FIFO-структуру. Разделение топика на разделы даёт возможность распределять нагрузку между брокерами, и обрабатывать их параллельно, обеспечивая масштабируемость и отказоустойчивость

---

**• Средний приоритет**

## 6. Поясните, что такое partition в Kafka?
...
- **_(опционально)_ Key** - ключ, применяется для распределения сообщения по партициям, не обязан быть уникальным
...
## 10. Поясните: Смещение (offset) в Kafka
...
- Ручное управление (manual commit) - потребитель сам обновляет offset после успешной обработки сообщения, применяется для большей надежности
```java
...
## 11. Поясните: Распределение сообщений. Как сообщения распределяются по партициям?
...
## 12. Зачем нужен интерфейс Partitioner?
...
## 13. Поясните: Гарантии доставки в Kafka
...
## 14. Поясните: Acknowledgment в Kafka
...
## 15. Каким образом гарантировать в Kafka идемпотентность, чтобы не было задвоений и потерь сообщений?
...
1. **Предотвращение дублирования сообщений - включение идемпотентности у продюсера:**
    Параметр `enable.idempotence` в конфигурации продюсера даёт возможность Kafka отслеживать дубликаты сообщений с использованием уникального идентификатора производителя (**Producer ID**) и последовательного номера сообщения (**Sequence Number**)
```java
...
## 16. Каким образом в Kafka гарантируется последовательность сообщений?
...
## 17. Зачем нужен Broker log cleaner thread?
...
только последнее обновление для данного ключа, к примеру, для обслуживания changelog или состояния БД. Программа очистки журналов 
...
## 18. Поясните: DLQ topic?

Dead Letter Queue (DLQ) - это отдельный Kafka-топик, куда отправляются сообщения с ошибками

**Зачем нужен DLQ?**
- Если консьюмер не может обработать сообщение (к примеру, ошибка десериализации)
...
## 19. Зачем нужен Consumer heartbeat thread?
...
## 20. Поясните: Что сделать, чтобы сообщения попадали в одну партицию?
...
## 21. Поясните: Архитектура топика
...
* **Партиции хранятся на диске** — Kafka сохраняет данные на диск, что даёт возможность долговременно хранить сообщения
...
## 22. Поясните: Настройки топика Kafka
...
## 23. Поясните, в чём разница между Kafka Consumer и Kafka Stream?

**Kafka Consumer** - это клиент, который читает данные из топика и производит некоторую обработку. Обычно применяется для
...
## 24. Поясните: Какова роль Consumer API?

Обеспечивает механизм для потребления сообщений топиков. Оно даёт возможность приложениям и микросервисам читать данные, 
...
## 25. Поясните: Какова роль Producer API?
...
## 26. Поясните: Пример конфигурации Kafka Consumer
...
такого режима в Spring Cloud Stream потребуется дополнительная логика, к примеру, фильтрация дублированных сообщений или
...
## 27. Поясните: Пример конфигурации Kafka Producer
...
## 28. Поясните: Брокеры сообщений vs Rest
...
## 29. Поясните: Какова роль AdminClient API?
...
## 30. Поясните: Какова роль Connector API?
...
## 31. Поясните: Какова роль Streams API?
...
## 32. Поясните: Какова роль Transactions API?

Kafka Transactions API даёт возможность выполнять атомарные обновления для нескольких топиков. Он включает exactly-once
...
## 33. Поясните: Плюсы и минусы брокеров сообщений
...
## 34. Поясните: Pull и push модели, в чём разница?
...
- удобно для реального времени (к примеру, уведомления)
**Недостатки:**
...
## 35. Поясните: Повторное чтение данных из Kafka

Kafka даёт возможность читать данные повторно, так как сообщения не удаляются сразу после прочтения
...
## 36. Каким образом работает Round Robin?
...
## 37. Какие именно существуют паттерны работы с очередями?
...
## 40. Поясните: Архитектура брокера
...
## 41. Поясните: Архитектура консюмера
...
Kafka использует концепцию Consumer Groups, что даёт возможность нескольким потребителям работать вместе, чтобы параллельно
...
## 42. Поясните: Архитектура продюсера

* **Создание сообщения (Record)**: Продюсер формирует сообщение, содержащее ключ (необязательный), значение и метаданные,
  такие как время отправки. Сообщение отправляется в топик (Topic), который включает в себя одной или нескольких партиций
...
## 43. Поясните, в чём разница между Kafka Streams и Apache Flink?
...
- Если вам нужно встроить обработку потоков данных в существующую Java/Scala программу без нужности развертывания отдельных кластеров.
...
## 44. Поясните, в чём разница между Kafka и Flume?
...
и низкую задержку для обработки больших объемов данных. Kafka применяется для создания стриминговых приложений и обработки 
...
## 45. Поясните, в чём разница между Kafka и RabbitMQ?

**RabbitMQ** и **Apache Kafka** — это две популярные системы обмена сообщениями, каждая из которых имеет свои особенности 
и применяется для разных типов приложений. Вот основные различия между ними:
...
## 46. Зачем нужен Kafka Mirror Maker?
...
## 47. Зачем нужен Streams DSL?

Kafka Streams DSL предоставляет высокоуровневый API для операций потоковой обработки. Он даёт возможность разработчикам описывать 
...
## 48. Зачем нужен идемпотентный продюсер?
...
## 49. Зачем нужен координатор группы?
...
## 51. Зачем нужна Schema Registry?
...
## 52. Зачем нужны методы commitSync() и commitAsync()?

Эти методы применяются для фиксации смещений:
...
## 53. Зачем нужны методы subscribe() и poll()?

Метод subscribe() применяется для подписки на один или несколько топиков. Фактически он не извлекает никаких данных.
...
## 54. Каким образом Kafka обеспечивает версионирование сообщений?
...
управления версиями используются реестры схем (к примеру, Confluent Schema Registry), которые могут управлять изменением схемы и совместимостью.

---

**• Низкий приоритет**

## 55. Каким образом Kafka обрабатывает задержку консюмера?
...
## 56. Каким образом Kafka обрабатывает сообщения?
...
* **Publish-Subscribe**: все сообщения обрабатываются всеми потребителями. Это достигается за счет того, что каждый
  потребитель находится в своей собственной группе потребителей, что даёт возможность всем потребителям читать все сообщения.

---

**• Низкий приоритет**

## 57. Каким образом потребители получают сообщения от брокера?

Kafka использует pull-модель для извлечения сообщений. Потребители запрашивают сообщения у брокеров, а не брокеры 
отправляют сообщения потребителям. Это даёт возможность потребителям контролировать скорость, с которой они получают сообщения. 
...
## 58. Поясните: Какова роль Quota API?

Quota API даёт возможность настраивать квоты для каждого клиента для ограничения скорости создания или потребления данных, чтобы 
...
## 59. Поясните: Настройки брокера Kafka
...
## 60. Поясните: Настройки консюмера
...
* **client.rack** — применяется для указания серверной стойки или дата-центра. Это особенно важно в случае, если у вас
...
## 61. Поясните: Настройки продюсера
...
* **Зачем это нужно**: Kafka продюсер использует эти брокеры для получения метаданных о кластере (к примеру, информация о топиках и партициях). Эти брокеры служат точками входа в кластер Kafka.
...
## 62. Поясните: Преимущества очередей
