...
- **Статья:** [EXPLAIN ANALYZE: как находить узкие места PostgreSQL](https://habr.com/ru/companies/first/articles/1006278/)
- **Статья:** [Почему PostgreSQL не использует ваш индекс](https://habr.com/ru/articles/1011998/)
- **Видео:** [Оптимизация PostgreSQL: индексы, EXPLAIN, планы выполнения](https://www.youtube.com/watch?v=gA3A_epB3So)

---

<span class="priority-badge priority-high">• Высокий приоритет</span>

## 1. Поясните: Уровни изолированности (read uncommitted, read committed, repeatable read, serializable)

<div class="answer-block">

Кратко: [Статья на хабре про уровни изоляции](https://habr.com/ru/articles/845522/)
...
## 2. Поясните: Оптимизация запросов. Какие существуют инструменты?

<div class="answer-block">

- **EXPLAIN** - показывает теоретический план выполнения запроса, что даёт возможность увидеть, как СУБД **планирует** извлекать данные
...
## 3. Поясните: Индексы в SQL. Что это?

<div class="answer-block">

Индексы в БД - это инструмент оптимизации/ускорения запросов путём создания упорядоченного набора указателей на строки таблицы. Ускорение происходит за счёт того, что структура индекса оптимизирована под поиск (к примеру, b-tree). При этом, чем больше дубликатов в столбце - тем хуже работает индекс
...
## 4. Поясните: Блокировки и локи. Оптимистичная и Пессимистичная

<div class="answer-block">

Кратко: Блокировки (или локи) в базе данных - это механизм, управляющий конкурентным доступом к данным, чтобы избежать конфликтов и ошибок. Есть два типа блокировок
...
## 5. Какие именно виды индексов знаешь?
...
## 6. Расскажите подробнее про ACID

<div class="answer-block">

Кратко: ACID - это набор свойств, гарантирующих надёжность транзакции:
...
## 7. Поясните: Виды JOIN. Как работает каждый JOIN?

<div class="answer-block">

Кратко: Оператор, позволяющий связать данные из двух таблиц. Бывают:
...
## 8. Поясните, что такое транзакции?

<div class="answer-block">

Кратко: Транзакция - это набор логически связанных запросов, выполняемых атомарно (либо выполняются все - commit транзакции, либо откат всего - rollback)
...
## 9. Какие инструменты применяют для оптимизации SQL-запросов?
...
## 11. Поясните: EXPLAIN vs EXPLAIN ANALYZE

<div class="answer-block">

Кратко: EXPLAIN показывает план выполнения запроса до его фактического выполнения. Он выводит, как СУБД собирается получить данные. Это включает:
...
## 12. Поясните: Проблемы параллельных транзакций
...
## 13. Поясните: Плюсы и минусы индексов
...
## 14. Поясните: Нормализация и денормализация. Перечислите формы
...
Денормализация - процесс обратный нормализации. Обычно это нужно для повышения производительности и скорости извлечения данных, за счет увеличения избыточности данных (влечёт уменьшение сложности запросов)
...
## 15. Поясните, чем WHERE отличается от HAVING? Можем ли мы их использовать в одном запросе?

<div class="answer-block">

Кратко: WHERE отвечает за фильтр всех записей в запросе (до применения агрегатных функций), HAVING применяет фильтры к группам, сформированным GROUP BY
...
## 16. По какой причине когда много индексов, это плохо?
...
## 17. Поясните: C помощью SQL блокировка строки? - SELECT FOR UPDATE

<div class="answer-block">

**`SELECT FOR UPDATE`** применяется для блокировки строк в таблице, участвующих в запросе до окончания транзакции, чтобы предотвратить их изменение другими транзакциями
```sql
BEGIN;
...
## 18. Поясните: View, materialized view. Разница между обычным и материализованным представлением?

<div class="answer-block">

View (представление) - это сохранённый SQL-запрос, который служит для удобного (без нужности повторно писать запрос) получения определённой выборки данных. Не хранит данные
```sql
...
## 19. Каким образом правильно выбрать индекс?
...
## 20. Поясните: Foreign Key

<div class="answer-block">

Кратко: Foreign Key (внешний ключ) - это колонка (или набор колонок) таблице, ссылающаяся на первичный ключ (или любую UNIQUE колонку) другой таблицы. Обеспечивает ссылочную целостность.
...
## 21. Поясните, что такое селективность?

<div class="answer-block">

Селективность - это характеристика избирательности запроса. Если значения в колонке варьируются в небольшом промежутке, то селективность запроса, выбирающего значения по этой колонке - низкая. Высокая селективность даёт возможность сильно ускорить процесс вычитывания данных
...
## 22. Поясните: Оконные функции. PARTITION BY
...
```
- `PARTITION BY <столбцы_для_разделения>` делит выборку на непересекающиеся подмножества, где каждое подмножество содержит строки с одинаковыми значениями в одном или нескольких столбцах, образуются партиции. Аналог `GROUP BY`, но не агрегирует строки, а даёт возможность работать с ними индивидуально в пределах группы
...
## 24. Поясните: БД. Constraint. Какие существуют?
...
## 25. Зачем используется Database Connection Pool? (Hikari) Исчерпание connection pool
...
Более того, ограничение кол-ва подключений в пуле даёт возможность исключить перегрузку базы данных множественными подключениями. При отсутствии свободных подключений желающий обратиться к БД обязан ждать освобождения одного из подключений пула.
...
## 26. Поясните: Хранимые процедуры
...
## 27. Поясните: UNION vs UNION ALL

<div class="answer-block">

Кратко: `UNION` и `UNION ALL` объединяют результаты двух или более запросов. Разница в том, что `UNION` осуществляет поиск и исключение дублирующихся строк, а `UNION ALL` - нет. В следствие этого `UNION` может работать медленнее. Оба оператора требуют, чтобы количество и типы столбцов в объединяемых выборках совпадали
```sql
...
## 28. Поясните: DELETE vs TRUNCATE
...
## 30. Расскажи подробнее про ключевое слово DISTINCT?

<div class="answer-block">

Кратко: `DISTINCT` - это ключевое слово SQL, служащее для выбора только уникальных записей, т.е. устраняет дублирующиеся строки из результата запроса
...
## 31. Поясните: cube, rollup, groupBy
...
**`GROUP BY`** применяется для группировки строк по определённым столбцам и применения агрегатных функций к каждой группе. Это позволяет выполнять расчёты по группам данных, например, подсчёт суммы, среднего значения и т.д.
...
## 32. Поясните: Инструмент для сбора статистики в PostgreSQL

<div class="answer-block">

Кратко: В PostgreSQL используется встроенный модуль pg_stat_statements для сбора статистики по запросам. Сохраняет информацию о запросах, частоте и времени их выполнения, а также сколько ресурсов потребляют
...
## 33. Поясните: Ключевое слово LATERAL

<div class="answer-block">

LATERAL даёт возможность использовать данные из каждой строки внешней таблицы в подзапросе (аналог “для каждой строки”)
...
## 34. Поясните, что такое внешний ключ?

<div class="answer-block">

Кратко: Внешний ключ - это столбец-ссылка на первичный ключ (или уникальную колонку) другой таблицы. Может быть null.
...
## 35. Поясните, что такое табличные выражения?

<div class="answer-block">

Кратко: Обобщённое табличное выражение или **CTE (Common Table Expressions)** - это именованный временный результирующий набор данных, к которому можно обращаться в последующих запросах. Реализовано через ключевое слово WITH
...
Кратко: Когда вы выполняете какой-нибудь запрос, оптимизатор запросов MySQL пытается придумать оптимальный план выполнения этого запроса. Можно посмотреть этот план используя запрос с ключевым словом EXPLAIN перед оператором SELECT.
...
## 37. Поясните, в чём отличие между операторами `DELETE` и `TRUNCATE`?

<div class="answer-block">

Кратко: `DELETE` - оператор DML, удаляет записи из таблицы, которые удовлетворяют критерию `WHERE` при этом задействуются триггеры, ограничения и т.д.
...
## 39. Поясните, в чём разница между `COUNT(*)` и `COUNT({column})`?

<div class="answer-block">

Кратко: `COUNT (*)` подсчитывает количество записей в таблице, не игнорируя значение NULL, поскольку эта функция оперирует записями, а не столбцами.
...
## 40. Поясните, в чём разница между операторами `GROUP BY` и `DISTINCT`?

<div class="answer-block">

Кратко: `DISTINCT` указывает, что для вычислений используются только уникальные значения столбца. `NULL` считается как отдельное значение. 
...
JOIN - оператор языка SQL, который является реализацией операции соединения реляционной алгебры. 
        ...
        - при нужности соединения не двух, а нескольких таблиц, операция соединения применяется 
...
## 42. Поясните: Для каких числовых типов недопустимо использовать операции сложения/вычитания?

<div class="answer-block">

Кратко: В качестве операндов операций сложения и вычитания нельзя использовать числовой тип `BIT`.
...
## 43. Зачем используется оператор `GROUP BY`?

<div class="answer-block">

`GROUP BY` применяется для агрегации записей результата по заданным признакам-атрибутам.
...
## 44. Зачем используется оператор `HAVING`?

<div class="answer-block">

`HAVING` применяется для фильтрации результата `GROUP BY` по заданным логическим условиям.
...
## 45. Зачем используется оператор `ORDER BY`?

<div class="answer-block">

Кратко: __ORDER BY__ упорядочивает вывод запроса согласно значениям в том или ином количестве выбранных столбцов. Многочисленные столбцы упорядочиваются один внутри другого. Возможно определять возрастание `ASC` или убывание `DESC` для каждого столбца. По умолчанию установлено - возрастание.
...
## 46. Зачем используются операторы `IN`, `BETWEEN`, `LIKE`?
...
+ `_` замещает любой одиночный символ. К примеру, `'b_t'` будет соответствовать словам `'bat'` или `'bit'`, но не будет соответствовать `'brat'`. 
...
## 47. Зачем используются операторы `INTERSECT`, `EXCEPT` в Transact-SQL?

<div class="answer-block">

Кратко: Оператор `EXCEPT` возвращает уникальные записи из левого входного запроса, которые не выводятся правым входным запросом.
...
Индексы относятся к методу настройки производительности, позволяющему быстрее извлекать записи из таблицы. Индекс создает структуру для индексируемого поля. Нужно просто добавить указатель индекса в таблицу.
...
## 50. Каким образом `GROUP BY` обрабатывает значение `NULL`?

<div class="answer-block">

Кратко: При использовании `GROUP BY` все значения `NULL` считаются равными.
...
## 51. Каким образом сделать запрос из двух баз?

<div class="answer-block">

Кратко: Если в запросе таблица указывается с именем базы данных database1.table1, то таблица выбирается из database1, если просто table1, то - из активной базы данных.
...
## 52. Каким образом создать индекс?

<div class="answer-block">

Кратко: Индекс можно создать либо с помощью выражения `CREATE INDEX`: 
```sql
CREATE INDEX index_name ON table_name (column_name)
```
...
## 53. Какие именно агрегатные функции вы знаете?

<div class="answer-block">

Кратко: Агрегатных функции - функции, которые берут группы значений и сводят их к одиночному значению.
...
## 54. Какие именно ограничения на целостность данных существуют в SQL?

<div class="answer-block">

`PRIMARY KEY` - набор полей (1 или более), значения которых образуют уникальную комбинацию и используются для однозначной идентификации записи в таблице. Для таблицы может быть создано только одно такое ограничение. Данное ограничение применяется для обеспечения целостности сущности, которая описана таблицей.
...
## 55. Какие именно отличия между ограничениями `PRIMARY` и `UNIQUE`?

<div class="answer-block">

Кратко: По умолчанию ограничение `PRIMARY` создает кластерный индекс на столбце, а `UNIQUE` - некластерный. Другим отличием является то, что `PRIMARY` не разрешает `NULL` записей, в то время как `UNIQUE` разрешает одну (а в некоторых СУБД несколько) `NULL` запись.
...
## 56. Какие именно существуют операторы SQL?

<div class="answer-block">

Кратко: __операторы определения данных (Data Definition Language, DDL)__:
...
## 57. Какие именно существуют типы `JOIN`?

<div class="answer-block">

Кратко: __(INNER) JOIN__
...
## 58. Поясните: Каков общий синтаксис оператора `SELECT`?

<div class="answer-block">

Кратко: `SELECT` - оператор DML SQL, возвращающий набор данных (выборку) из базы данных, удовлетворяющих заданному условию. Имеет следующую структуру:
...
## 60. Поясните: Может ли значение в столбце, на который наложено ограничение `FOREIGN KEY`, равняться `NULL`?

<div class="answer-block">

Кратко: Может, если на данный столбец не наложено ограничение `NOT NULL`.
...
## 61. Поясните: Напишите запрос...

<div class="answer-block">

```sql
Кратко: CREATE TABLE table ( 
...
## 62. Поясните: Нюансы работы с NULL в SQL. Как проверить поле на NULL?

<div class="answer-block">

Кратко: NULL - специальное значение (псевдозначение), которое может быть записано в поле таблицы базы данных. NULL соответствует понятию «пустое поле», то есть «поле, не содержащее никакого значения».
...
## 63. Поясните: Опишите разницу типов данных `DATETIME` и `TIMESTAMP`.

<div class="answer-block">

`DATETIME` служит для хранения целого числа: `YYYYMMDDHHMMSS`. И это время не зависит от временной зоны, настроенной на сервере.
Размер: 8 байт
...
## 64. Назовите основные агрегатные функции.

<div class="answer-block">

Кратко: __Агрегатных функции__ - функции, которые берут группы значений и сводят их к одиночному значению. 
...
## 65. Поясните: Расскажите об основных функциях ранжирования в Transact-SQL.

<div class="answer-block">

Ранжирующие функции - это функции, которые возвращают значение для каждой записи группы в результирующем наборе данных. На практике они могут быть использованы, к примеру, для простой нумерации списка, составления рейтинга или постраничной навигации.
...
## 66. Расскажите подробнее про операторы IN, BETWEEN, LIKE.
...
## 67. Расскажите подробнее про уровни изолированности транзакций.

<div class="answer-block">

Кратко: В порядке увеличения изолированности транзакций и, соответственно, надежности работы с данными:
...
## 68. Поясните, чем TRUNCATE отличается от DELETE?

<div class="answer-block">

Кратко: DELETE - оператор DML, удаляет записи из таблицы, которые удовлетворяют условиям WHERE. Медленнее, чем TRUNCATE. Есть возможность восстановить данные.
...
## 69. Поясните, чем WHERE отличается от HAVING ( ответа про то что используются в разных частях запроса - недостаточно)?

<div class="answer-block">

WHERE нельзя использовать с агрегатными функциями, HAVING можно (предикаты тоже).
В HAVING можно использовать псевдонимы только если они применяются для наименования результата агрегатной функции, в WHERE можно всегда.
...
## 70. Поясните, чем отличается MySQL от PostgreSQL?
...
## 71. Поясните: Что быстрее убирает дубликаты distinct или group by?

<div class="answer-block">

Кратко: Если нужны уникальные значения - DISTINCT.
...
## 73. Поясните: Что делает оператор `EXISTS`?

<div class="answer-block">

Кратко: `EXISTS` берет подзапрос, как аргумент, и оценивает его как `TRUE`, если подзапрос возвращает какие-либо записи и `FALSE`, если нет.
...
## 74. Поясните: Что делает оператор `MERGE`?

<div class="answer-block">

`MERGE` даёт возможность осуществить слияние данных одной таблицы с данными другой таблицы. При слиянии таблиц проверяется условие, и если оно истинно, то выполняется `UPDATE`, а если нет - `INSERT`. При этом изменять поля таблицы в секции `UPDATE`, по которым идет связывание двух таблиц, нельзя.
...
## 75. Поясните: Что делает оператор MERGE? Какие у него есть ограничения?
...
## 76. В каких случаях использовать JOIN, а в каких — подзапрос?

<div class="answer-block">

Кратко: Обычно лучше использовать `JOIN`, поскольку в большинстве случаев он более понятен и лучше оптимизируется СУБД (но 100% этого гарантировать нельзя). Так же `JOIN` имеет заметное преимущество над подзапросами в случае, когда список выбора `SELECT` содержит столбцы более чем из одной таблицы.
...
## 77. Поясните: Что означает `NULL` в SQL?
...
## 78. Поясните, что такое _«SQL»_?

<div class="answer-block">

Кратко: SQL, Structured query language («язык структурированных запросов») — формальный непроцедурный язык программирования, применяемый для создания, модификации и управления данными в произвольной реляционной базе данных, управляемой соответствующей системой управления базами данных (СУБД).
...
## 79. Поясните, что такое _«временная таблица»_? Для чего она используется?

<div class="answer-block">

Кратко: __Временная таблица__ - это объект базы данных, который хранится и управляется системой базы данных на временной основе. Они могут быть локальными или глобальными. Используется для сохранения результатов вызова хранимой процедуры, уменьшение числа строк при соединениях, агрегирование данных из различных источников или как замена курсоров и параметризованных представлений.
...
## 80. Поясните, что такое _«курсор»_?

<div class="answer-block">

__Курсор__ — это объект базы данных, который даёт возможность приложениям работать с записями «по-одной», а не сразу с множеством, как это делается в обычных SQL командах.
...
## 81. Поясните, что такое _«представление» (view)_ и для чего оно применяется?

<div class="answer-block">

Кратко: __Представление__, View - виртуальная таблица, представляющая данные одной или более таблиц альтернативным образом.
...
## 82. Поясните, что такое _«хранимая процедура»_?

<div class="answer-block">

Кратко: __Хранимая процедура__ — объект базы данных, представляющий собой набор SQL-инструкций, который хранится на сервере. Хранимые процедуры очень похожи на обыкновенные процедуры языков высокого уровня, у них могут быть входные и выходные параметры и локальные переменные, в них могут производиться числовые вычисления и операции над символьными данными, результаты которых могут присваиваться переменным и параметрам. В хранимых процедурах могут выполняться стандартные операции с базами данных (как DDL, так и DML). Кроме того, в хранимых процедурах возможны циклы и ветвления, то есть в них могут использоваться инструкции управления процессом исполнения.
...
+ при нужности соединения не двух, а нескольких таблиц, операция соединения применяется несколько раз (последовательно).
...
## 84. Поясните, что такое DCL? Какие операции в него входят? Рассказать про них.

<div class="answer-block">

Кратко: операторы определения доступа к данным (Data Control Language, DCL):
GRANT предоставляет пользователю (группе) разрешения на определенные операции с объектом,
REVOKE отзывает ранее выданные разрешения,
DENY задает запрет, имеющий приоритет над разрешением;
...
## 85. Поясните, что такое DDL? Какие операции в него входят? Рассказать про них.

<div class="answer-block">

Кратко: DDL (Data Definition Language) -  операторы определения данных (Data Definition Language, DDL):
CREATE создает объект БД (базу, таблицу, представление, пользователя и т. д.),
ALTER изменяет объект,
DROP удаляет объект;
...
## 86. Поясните, что такое DISTINCT?

<div class="answer-block">

Кратко: DISTINCT указывает, что для вычислений используются только уникальные значения столбца.
...
## 87. Поясните, что такое DML? Какие операции в него входят? Рассказать про них.

<div class="answer-block">

Кратко: операторы манипуляции данными (Data Manipulation Language, DML):
SELECT выбирает данные, удовлетворяющие заданным условиям,
INSERT добавляет новые данные,
UPDATE изменяет существующие данные,
...
## 89. Поясните, что такое GROUP BY?

<div class="answer-block">

GROUP BY применяется для агрегации записей результата по заданным атрибутам.
...
## 90. Поясните, что такое LIMIT?

<div class="answer-block">

Кратко: Ограничивает выборку заданным числом.
...
## 91. Поясните, что такое ORDER BY?

<div class="answer-block">

Кратко: ORDER BY упорядочивает вывод запроса согласно значениям в том или ином количестве выбранных столбцов. Многочисленные столбцы упорядочиваются один внутри другого. Возможно определять возрастание ASC или убывание DESC для каждого столбца. По умолчанию установлено - возрастание.
...
## 92. Поясните, что такое TCL? Какие операции в него входят? Рассказать про них.

<div class="answer-block">

Кратко: операторы управления транзакциями (Transaction Control Language, TCL):
...
## 93. Поясните, что такое TIMESTAMP?

<div class="answer-block">

DATETIME служит для хранения целого числа: YYYYMMDDHHMMSS. И это время не зависит от временной зоны настроенной на сервере. Размер: 8 байт
...
## 94. Поясните, что такое временные таблицы? Для чего они нужны?

<div class="answer-block">

Кратко: Подобные таблицы удобны для каких-то временных промежуточных выборок из нескольких таблиц.
...
## 95. Поясните, что такое нормализация и денормализация? Расскажите про 3 нормальные формы?
...
## 96. Поясните, что такое ограничения (constraints)? Какие вы знаете?

<div class="answer-block">

Кратко: Ограничения - это ключевае слова, которые помогают установить правила размещения данных в базе. Используются при создании БД.
...
## 97. Поясните, что такое представления (VIEW)? Для чего они нужны?

<div class="answer-block">

Кратко: View - виртуальная таблица, представляющая данные одной или более таблиц альтернативным образом.
В действительности представление – всего лишь результат выполнения оператора SELECT, который хранится в структуре памяти, напоминающей SQL таблицу. Они работают в запросах и операторах DML точно также как и основные таблицы, но не содержат никаких собственных данных. Представления значительно расширяют возможности управления данными. Это способ дать публичный доступ к некоторой (но не всей) информации в таблице.
...
## 98. Поясните, что такое суррогатные ключи?

<div class="answer-block">

Кратко: Суррога́тный ключ — это дополнительное служебное поле, автоматически добавленное к уже имеющимся информационным полям таблицы, предназначение которого — служить первичным ключом.
...
## 99. Поясните, что такое транзакции? Расскажите про принципы ACID.

<div class="answer-block">

Кратко: Транзакция - это воздействие на базу данных, переводящее её из одного целостного состояния в другое и выражаемое в изменении данных, хранящихся в базе данных.
...
## 101. Поясните: Шардирование БД

<div class="answer-block">

При большом количестве данных запросы начинают долго выполняться, и сервер начинает не справляться с нагрузкой. Одно из решений, что с этими данными делать — это масштабирование базы данных. К примеру, шардинг или репликация. 
