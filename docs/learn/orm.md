...
- **Статья:** [Hibernate: сессии и транзакции](https://habr.com/ru/articles/271115/)
- **Видео:** [Собеседование Middle Java: ORM, Hibernate, Entity, N+1](https://www.youtube.com/watch?v=EQG9bzlMUrE)

---

<span class="priority-badge priority-high">• Высокий приоритет</span>

## 1. Поясните: Проблема N + 1. Способы решения проблемы
...
## 2. Поясните, что такое Proxy и для чего используется?
...
## 4. Поясните, что такое Hibernate, JPA, JDBC, ORM?
...
## 5. Поясните: Жизненный цикл сущности

<div class="answer-block">

Кратко: В Hibernate сущность может находиться в одном из четырех состояний:
- Transient
- Persistent
- Detached
- Removed
...
## 6. Поясните: LazyInitializationException

<div class="answer-block">

Кратко: Это происходит, когда объект был загружен с ленивой загрузкой, но его данные пытаются быть использованы после закрытия сессии
...
## 7. Поясните, что такое Entity Graph?
...
## 9. Поясните: Связи между таблицами в ORM

<div class="answer-block">

Кратко: 1. OneToOne - один экземпляр сущности может быть связан не больше чем с одним экземпляром другой сущности
...
## 10. Поясните: Стратегии наследования в Hibernate
...
## 11. Поясните: FetchType

<div class="answer-block">

Кратко: `LAZY` - вместо таких полей будут вставляться прокси, а сами они будут подгружаться при первом доступе к ним
...
## 12. Поясните: Hibernate. Что хранится в Persistence Context? Сколько времени объект хранится в Persistence Context?
...
## 13. Поясните: Statement, Prepared statement, Callable statement

<div class="answer-block">

- Statement - объект, который применяется для формирования простых SQL запросов. Уязвим к SQL-инъекциям
...
## 14. Каким образом создать Entity и что это такое?
...
## 15. Поясните: EntityManager + EntityManagerFactory / SessionFactory + session
...
## 16. Поясните: Аннотация @Query, что это такое?
...
## 17. Поясните, что такое SQL-инъекция?
...
## 18. Поясните: Hibernate и Lombok
...
## 20. Каким образом JPA делает запросы?
...
## 21. Поясните: Проблемы JOIN FETCH
...
## 22. Каким образом Hibernate генерирует SQL запросы?

<div class="answer-block">

Кратко: Hibernate генерирует SQL-запросы для выполнения операций с сущностями на основе конфигурации и аннотаций, таких как `@Entity`, `@OneToMany`, `@ManyToOne`, и других.
...
## 24. По какой причине мы не можем использовать final классы в качестве Entity?

<div class="answer-block">

Кратко: Потому что Hibernate делает прокси через наследование от класса-entity, `final` прямо этому препятствует.
...
## 25. Поясните: Dirty checking механизм Hibernate
...
## 26. Зачем нужна аннотация @Cache?
...
## 27. Зачем нужна аннотация @Cacheable?
...
## 28. Зачем нужна аннотация Access?

<div class="answer-block">

Кратко: Она определяет тип доступа (access type) для класса entity, суперкласса, embeddable или отдельных атрибутов, то есть как JPA будет обращаться к атрибутам entity, как к полям класса (FIELD) или как к свойствам класса (PROPERTY), имеющие гетеры (getter) и сетеры (setter).
...
## 29. Зачем нужна аннотация Basic?

<div class="answer-block">

Кратко: @Basic - указывает на простейший тип маппинга данных на колонку таблицы базы данных. Также в параметрах аннотации можно указать fetch стратегию доступа к полю и является ли это поле обязательным или нет. Может быть применена к полю любого из следующих типов:
...
## 30. Зачем нужна аннотация Column?
...
## 31. Зачем нужна аннотация ID? Какие @GeneratedValue вы знаете?
...
## 32. Зачем нужна аннотация Transient?
...
## 33. Зачем нужны аннотации @Embedded и @Embeddable?
...
## 34. Зачем нужны аннотации @OrderBy и @OrderColumn, чем они отличаются?
...
## 35. Каким образом влияет операция detach на Entity объекты каждого из четырех статусов?

<div class="answer-block">

Кратко: managed, removed → detached.
...
## 36. Каким образом влияет операция merge на Entity объекты каждого из четырех статусов?

<div class="answer-block">

Кратко: new → будет создан новый managed entity, в который будут скопированы данные прошлого объекта
...
## 37. Каким образом влияет операция persist на Entity объекты каждого из четырех статусов?

<div class="answer-block">

Кратко: new → managed, и объект будет сохранен в базу при commit-е транзакции или в результате flush операций
...
## 38. Каким образом влияет операция refresh на Entity объекты каждого из четырех статусов?

<div class="answer-block">

Кратко: managed → будут восстановлены все изменения из базы данных данного Entity, также произойдет refresh всех каскадно зависимых объектов
...
## 39. Каким образом влияет операция remove на Entity объекты каждого из четырех статусов?

<div class="answer-block">

Кратко: new → операция игнорируется, однако зависимые Entity могут поменять статус на removed, если у них есть аннотации каскадных изменений и они имели статус managed
...
## 40. Каким образом мапятся Enum'ы?

<div class="answer-block">

Кратко: @Enumerated(EnumType.STRING) - означает, что в базе будут храниться имена Enum.
...
## 41. Каким образом мапятся даты (до java 8 и после)?

<div class="answer-block">

Кратко: Аннотация @Temporal до Java 8, в которой надо было указать какой тип даты мы хотим использовать.
...
## 42. Каким образом работать с кешем 2 уровня?

<div class="answer-block">

Кратко: Чтение из кэша второго уровня происходит только в том случае, если нужный объект не был найден в кэше первого уровня.
...
## 43. Каким образом смапить составной ключ?
...
## 44. Каким образом “смапить” коллекцию примитивов?
...
## 45. Какие именно два вида кэшей (cache) вы знаете в JPA и для чего они нужны?
...
## 46. Какие именно два типа fetch стратегии в JPA вы знаете?

<div class="answer-block">

Кратко: 1) LAZY — Hibernate может загружать данные не сразу, а при первом обращении к ним, но так как это необязательное требование, то Hibernate имеет право изменить это поведение и загружать их сразу. Это поведение по умолчанию для полей, аннотированных @OneToMany, @ManyToMany и @ElementCollection. В объект загружается прокси lazy-поля.
...
## 47. Какие именно есть виды связей?

<div class="answer-block">

Кратко: Существуют 4 типа связей:
...
## 48. Какие именно три типа стратегий наследования мапинга (Inheritance Mapping Strategies) описаны в JPA?
...
## 49. Какие именно четыре статуса жизненного цикла Entity объекта (Entity Instance’s Life Cycle) вы можете перечислить?

<div class="answer-block">

Кратко: Transient (New) — свежесозданная оператором new() сущность не имеет связи с базой данных, не имеет данных в базе данных и не имеет сгенерированных первичных ключей.
...
## 50. Какие именно шесть видов блокировок (lock) описаны в спецификации JPA (или какие есть значения у enum LockModeType в JPA)?
...
## 51. Поясните: Каким условиям должен удовлетворять класс чтобы являться Entity?
...
## 52. Поясните: Может ли Entity класс наследоваться от других Entity классов?
...
## 53. Поясните: Может ли Entity класс наследоваться от не Entity классов (non-entity classes)?
...
## 54. Поясните: Может ли абстрактный класс быть Entity?

<div class="answer-block">

Кратко: Может, при этом он сохраняет все свойства Entity, за исключением того что его нельзя непосредственно инициализировать.
...
## 55. Поясните: различия между Repository и DAO:

<div class="answer-block">

Кратко: краткий список тезисов, характеризующих различия между Repository и DAO:
...
## 56. Поясните: Разница между PERSIST и MERGE?

<div class="answer-block">

Кратко: persist(entity) следует использовать с совершенно новыми объектами, чтобы добавить их в БД (если объект уже существует в БД, будет выброшено исключение EntityExistsException).
...
## 57. Расскажите подробнее про аннотации @JoinColumn и @JoinTable? Где и для чего они используются?
...
## 58. Расскажите подробнее про проблему N+1 Select и путях ее решения.

<div class="answer-block">

Кратко: Проблема N+1 запросов возникает, когда получение данных из БД выполняется за N дополнительных SQL-запросов для извлечения тех же данных, которые могли быть получены при выполнении основного SQL-запроса.
...
## 59. Поясните, что такое Criteria API и для чего он используется?
...
## 60. Поясните, что такое EntityManager?

<div class="answer-block">

Кратко: EntityManager интерфейс JPA, который описывает API для всех основных операций над Entity, а также для получения данных и других сущностей JPA. 
 Основные операции:
...
## 61. Поясните, что такое JPQL/HQL и чем он отличается от SQL?

<div class="answer-block">

Кратко: Hibernate Query Language (HQL) и Java Persistence Query Language (JPQL) - оба являются объектно-ориентированными языками запросов, схожими по природе с SQL. JPQL - это подмножество HQL.
...
## 62. Поясните, что такое Mapped Superclass?
...
## 63. Поясните, что такое ORM? Что такое JPA? Что такое Hibernate?
...
## 64. Поясните, что такое владелец связи?

<div class="answer-block">

Кратко: В отношениях между двумя сущностями всегда есть одна владеющая сторона, а владеемой может и не быть, если это однонаправленные отношения.
...
## 65. Поясните, что такое встраиваемый (Embeddable) класс? Какие требования JPA устанавливает к встраиваемым (Embeddable) классам?

<div class="answer-block">

Кратко: Embeddable класс - это класс, который не используется сам по себе, а является частью одного или нескольких Entity-классов. Entity-класс может содержать как одиночные встраиваемые классы, так и коллекции таких классов. Также такие классы могут быть использованы как ключи или значения map. Во время выполнения каждый встраиваемый класс принадлежит только одному объекту Entity-класса и не может быть использован для передачи данных между объектами Entity-классов (то есть такой класс не является общей структурой данных для разных объектов). В целом, такой класс служит для того чтобы выносить определение общих атрибутов для нескольких Entity.
...
## 66. Поясните, что такое каскады?

<div class="answer-block">

Кратко: Каскадирование - это когда мы выполняем какое-то действие с целевой Entity, то же самое действие будет применено к связанной Entity.
JPA CascadeType:
