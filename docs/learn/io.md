...
## 1. Поясните, в чём проблема сериализации Singleton?

<div class="answer-block">

Кратко: Проблема в том что после десериализации мы получим другой объект. Таким образом, сериализация дает возможность создать Singleton еще раз, что недопустимо. Существует два способа избежать этого:
...
## 3. Каким образом исключить поля из сериализации?

<div class="answer-block">

Кратко: Для управления сериализацией при определении полей можно использовать ключевое слово `transient`, таким образом исключив поля из общего процесса сериализации.
...
## 4. Каким образом не допустить сериализацию?

<div class="answer-block">

Кратко: Чтобы не допустить автоматическую сериализацию можно переопределить `private` методы для создания исключительной ситуации `NotSerializableException`.
...
## 5. Каким образом создать собственный протокол сериализации?

<div class="answer-block">

Кратко: Для создания собственного протокола сериализации достаточно реализовать интерфейс `Externalizable`, который содержит два метода:
...
## 6. Поясните: Какая роль поля `serialVersionUID` в сериализации?

<div class="answer-block">

`serialVersionUID` применяется для указания версии сериализованных данных. 
...
## 7. Какие именно существуют способы контроля за значениями десериализованного объекта

<div class="answer-block">

Если есть нужность выполнения контроля за значениями десериализованного объекта, то можно использовать интерфейс `ObjectInputValidation` с переопределением метода `validateObject()`.
...
## 8. Поясните: Какое влияние оказывают на сериализуемость модификаторы полей `static` и `final`
...
## 9. В каких случаях стоит изменять значение поля `serialVersionUID`?

<div class="answer-block">

`serialVersionUID` нужно изменять при внесении в класс несовместимых изменений, к примеру при удалении какого-либо его атрибута.
...
## 10. Поясните: Опишите процесс сериализации/десериализации с использованием `Serializable`.
...
При этом ранее сериализованные объекты повторно не сериализуются, что даёт возможность алгоритму корректно работать с циклическими ссылками.
...
## 11. Поясните: Что обозначает ключевое слово `transient`?

<div class="answer-block">

Кратко: Поля класса, помеченные модификатором `transient`, не сериализуются.
...
## 12. Поясните, что такое _«сериализация»_?
...
Сериализация даёт возможность в определенных пределах изменять класс. Вот наиболее важные изменения, с которыми спецификация Java Object Serialization может справляться автоматически:
