...
- **Статья:** [HTTP API и REST: организация API по принципам REST](https://habr.com/ru/articles/742124/)
...
## 2. Поясните: REST vs RPC. Как выбрать что лучше подходит?
...
## 5. Перечислите 6 принципов REST API
...
## 7. Поясните: 6 принципов REST API
...
## 8. Поясните: В каких ситуациях допустимо использование RESTless
...
## 9. Что представляют собой HTTP и HTTPS и чем они различаются?
...
## 11. Поясните: Статусы ответа HTTP
...
## 12. Поясните: За что отвечают Cookie/куки?
...
## 13. Какие именно методы для работы с cookies предусмотрены в сервлетах?
...
## 14. Поясните, что такое _cookies_?
...
## 15. Поясните: Какой метод HTTP не является неизменяемым?
...
## 16. Поясните, в чём отличия `ServletContext` и `ServletConfig`?
...
## 17. Поясните, в чём разница между `PrintWriter` и `ServletOutputStream`?
...
## 18. Зачем нужен интерфейс `ServletRequest`?
...
## 19. Зачем нужен интерфейс `ServletResponse`?
...
## 20. Какие именно основные методы присутствуют в классе `HttpServlet`?
...
## 21. Какие именно основные особенности появились в спецификации _Servlet 3_?
...
## 22. Допустимо ли одновременно использовать в сервлете `PrintWriter` и `ServletOutputStream`?
...
## 23. По какой причине `HttpServlet` класс объявлен как абстрактный?
...
## 24. Поясните: Что представляет собой `ServletConfig`?
...
## 25. Поясните: Что представляет собой `ServletContext`?
...
## 26. Поясните: Основные HTTP методы
...
## 28. Поясните: Отличия POST, PUT, PATCH
...
## 29. Поясните, что такое идемпотентный HTTP-метод?
...
## 30. Поясните: Идемпотентный метод
...
## 31. Поясните: Из чего состоит HTTP запрос
...
## 32. Поясните: Из чего состоит JSON Web Token?
...
## 33. Какие именно методы авторизации существуют для HTTP протокола?
...
## 34. Какие именно протоколы уровня приложений?
...
## 37. Поясните: В каком случае требуется переопределять метод `service()`?
...
## 38. Поясните, в чём заключаются преимущества технологии сервлетов над CGI (Common Gateway Interface)?
...
## 39. Поясните, в чём разница между методами `GET` и `POST`?
...
## 40. Зачем используются атрибуты сервлетов и как происходит работа с ними?
...
## 41. Поясните: Есть ли смысл определять для сервлета конструктор? Каким образом лучше инициализировать данные?
...
## 42. Для чего в сервлетах используются различные _listener_?
...
## 43. Для чего нужны и чем отличаются методы `encodeURL()` и `encodeRedirectURL()`?
...
## 44. Для чего нужны сервера приложений, если есть контейнеры сервлетов?
...
## 45. Поясните: Защита от рефлексии
...
## 46. Поясните: Из каких групп тегов состоит библиотека _JSTL_?
...
## 47. Каким образом из одного сервлета вызвать другой сервлет?
...
## 48. Каким образом контейнер сервлетов управляет жизненным циклом сервлета, когда и какие методы вызываются?
...
## 49. Каким образом мы можем обеспечить _transport layer security_ для нашего веб приложения?
...
## 50. Каким образом обработать в приложении исключения, выброшенные другим сервлетом?
...
## 51. Каким образом организовать подключение к базе данных, обеспечить журналирование в сервлете?
...
## 52. Каким образом происходит обработка ошибок с помощью JSTL?
...
## 53. Каким образом реализовать запуск сервлета одновременно с запуском приложения?
...
## 54. Каким образом уведомить объект в сессии, что сессия недействительна или закончилась?
...
## 55. Какие именно действия необходимо проделать при создании сервлетов?
...
## 56. Какие именно есть методы отправки данных с клиента на сервер?
...
## 57. Какие именно классы-обертки для сервлетов вы знаете?
...
## 58. Какие именно наиболее распространенные задачи выполняются в контейнере сервлетов?
...
## 59. Какие именно различные методы управления сессией в сервлетах вы знаете?
...
## 60. Какие именно способы аутентификации доступны сервлету?
...
## 61. Какие именно существуют уровни модели _OSI_?
...
## 62. Какие именно типы EL операторов вы знаете?
...
## 63. Поясните: Каким образом можно допустить в сервлете deadlock?
...
## 64. Поясните: Какова структура веб-проекта?
...
## 65. Поясните: Какой существует эффективный способ удостоверится, что все сервлеты доступны только для пользователя с верной сессией?
...
## 66. В каких случаях стоит использовать фильтры сервлетов, а когда слушателей?
...
## 67. По какой причине необходимо переопределить только `init()` метод без аргументов?
...
## 68. Поясните: Приведите пример использования собственных тегов.
...
## 69. Поясните: Различие между методами GET и POST
...
## 70. Поясните: Расскажите об интерфейсе `SingleThreadModel`.
...
## 71. Поясните: Стоит ли волноваться о многопоточной безопасности работая с сервлетами?
...
## 72. Поясните: Часто встречающиеся коды ошибок HTTP
...
## 73. Поясните, чем SCRUM отличается от Kanban?
...
## 74. Поясните, чем отличается `sendRedirect()` от `forward()`?
...
## 75. Поясните, чем отличаются _TCP_ и _UDP_?
...
## 76. Поясните, чем отличаются _Web server_ и _Application server_?
...
## 77. Поясните, чем отличаются методы _GET_ и _POST_?
...
## 78. Поясните: Что вы знаете о _сервлетных фильтрах_?
...
## 79. Поясните: Что вы знаете о `PageContext` и какие преимущества его использования?
...
## 80. Поясните: Что означает _URL encoding_? Как это осуществить в Java?
...
## 81. Поясните, что такое _AJAX_? Как принципиально устроена эта технология?
...
## 82. Поясните, что такое _Application server_?
...
## 84. Поясните, что такое _JSON схема_?
...
## 86. Поясните, что такое _MIME тип_?
...
## 87. Поясните, что такое _TCP/IP_?
...
## 89. Поясните, что такое _URL Rewriting_?
...
## 91. Поясните, что такое _Web application_?
...
## 92. Поясните, что такое _Web server_?
...
## 93. Поясните, что такое _WebSocket_?
...
## 95. Поясните, что такое _«авторизация»_ и _«аутентификация»_? Чем они отличаются?
...
## 96. Поясните, что такое _«дескриптор развертывания»_?
...
## 97. Поясните, что такое _«контейнер сервлетов»_?
...
## 98. Поясните, что такое _«сервлет»_?
...
## 99. Поясните, что такое _«сессия»_?
...
## 100. Поясните, что такое `Request Dispatcher`?
...
## 101. Поясните, что такое протокол передачи данных? Какие протоколы вы знаете?
...
## 102. Поясните: Взаимодействие _JSP - сервлет - JSP_.
...
## 103. Поясните: Всегда ли создается объект сессии на JSP странице, можно ли отключить его создание?
...
## 105. Каким образом закомментировать код в JSP?
...
## 106. Каким образом конфигурируется JSP в дескрипторе развертывания.
...
## 107. Каким образом можно обработать ошибки JSP страниц?
...
## 108. Каким образом можно предотвратить прямой доступ к JSP странице из браузера?
...
## 109. Каким образом отключить возможность использования EL в JSP?
...
## 110. Каким образом сделать перенос строки в HTML средствами JSP?
...
## 111. Каким образом сконфигурировать параметры инициализации для JSP?
...
## 112. Поясните: Какая разница между _динамическим_ и _статическим_ содержимым JSP?
...
## 113. Поясните: Какая разница между `<c:set>` и `<jsp:useBean>`?
...
## 114. Поясните: Какая разница между `JSPWriter` и сервлетным `PrintWriter`?
...
## 115. Какие именно неявные объекты не доступны в обычной JSP странице?
...
## 116. Какие именно неявные, внутренние объекты и методы есть на JSP странице?
...
## 117. Какие именно области видимости переменных существуют в JSP?
...
## 118. Какие именно существуют основные типы тегов JSP?
...
## 119. Допустимо ли использовать Javascript на JSP странице?
...
## 120. Допустимо ли определить класс внутри JSP страницы?
...
## 121. Перечислите неявные, внутренние объекты JSP EL и их отличия от объектов JSP.
...
## 122. Поясните: Опишите общие практические принципы работы с JSP.
...
## 123. Поясните: Опишите, как обрабатываются JSP страницы, начиная от запроса к серверу, заканчивая ответом пользователю.
...
## 124. По какой причине не нужно конфигурировать стандартные JSP теги в `web.xml`?
...
## 125. По какой причине не рекомендуется использовать скриплеты (скриптовые элементы) в JSP?
...
## 126. Поясните: Расскажите о методах жизненного цикла JSP.
...
## 127. Поясните: Расскажите об этапах (фазах) жизненного цикла JSP.
...
## 128. Поясните, чем отличается `<c:import>` от `<jsp:include>` и директивы `<%@include %>`?
...
## 129. Поясните: Что вы знаете о действиях JSP (_Action tag_ и _JSP Action Elements_).
...
## 130. Поясните: Что вы знаете о Языке выражений JSP (JSP Expression Language – EL)?
...
## 131. Поясните, что такое _Java Server Pages (JSP)_?
...
## 132. Поясните, что такое _JSTL (JSP Standard tag library)_?
