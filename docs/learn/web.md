...
## 48. Каким образом происходит обработка ошибок с помощью JSTL?
...
## 49. Каким образом реализовать запуск сервлета одновременно с запуском приложения?
...
## 50. Каким образом сделать перенос строки в HTML средствами JSP?
...
## 51. Каким образом сконфигурировать параметры инициализации для JSP?
...
## 52. Каким образом уведомить объект в сессии, что сессия недействительна или закончилась?
...
## 53. Поясните: Какая разница между _динамическим_ и _статическим_ содержимым JSP?
...
## 54. Поясните: Какая разница между `<c:set>` и `<jsp:useBean>`?
...
## 55. Поясните: Какая разница между `JSPWriter` и сервлетным `PrintWriter`?
...
## 56. Какие именно действия необходимо проделать при создании сервлетов?
...
## 57. Какие именно есть методы отправки данных с клиента на сервер?
...
## 58. Какие именно классы-обертки для сервлетов вы знаете?
...
## 59. Какие именно методы для работы с cookies предусмотрены в сервлетах?
...
## 60. Какие именно наиболее распространенные задачи выполняются в контейнере сервлетов?
...
## 61. Какие именно неявные объекты не доступны в обычной JSP странице?
...
## 62. Какие именно неявные, внутренние объекты и методы есть на JSP странице?
...
## 63. Какие именно области видимости переменных существуют в JSP?
...
## 64. Какие именно основные методы присутствуют в классе `HttpServlet`?
...
## 65. Какие именно основные особенности появились в спецификации _Servlet 3_?
...
## 66. Какие именно различные методы управления сессией в сервлетах вы знаете?
...
## 67. Какие именно способы аутентификации доступны сервлету?
...
## 68. Какие именно существуют основные типы тегов JSP?
...
## 69. Какие именно существуют уровни модели _OSI_?
...
## 70. Какие именно типы EL операторов вы знаете?
...
## 71. Поясните: Каким образом можно допустить в сервлете deadlock?
...
## 72. Поясните: Какова структура веб-проекта?
...
## 73. Поясните: Какой метод HTTP не является неизменяемым?
...
## 74. Поясните: Какой существует эффективный способ удостоверится, что все сервлеты доступны только для пользователя с верной сессией?
...
## 75. В каких случаях стоит использовать фильтры сервлетов, а когда слушателей?
...
## 76. Допустимо ли использовать Javascript на JSP странице?
...
## 77. Допустимо ли одновременно использовать в сервлете `PrintWriter` и `ServletOutputStream`?
...
## 78. Допустимо ли определить класс внутри JSP страницы?
...
## 79. Перечислите неявные, внутренние объекты JSP EL и их отличия от объектов JSP.
...
## 80. Поясните: Опишите общие практические принципы работы с JSP.
...
## 81. Поясните: Опишите, как обрабатываются JSP страницы, начиная от запроса к серверу, заканчивая ответом пользователю.
...
## 82. Что представляют собой HTTP и HTTPS и чем они различаются?
...
## 83. По какой причине `HttpServlet` класс объявлен как абстрактный?
...
## 84. По какой причине не нужно конфигурировать стандартные JSP теги в `web.xml`?
...
## 85. По какой причине не рекомендуется использовать скриплеты (скриптовые элементы) в JSP?
...
## 86. По какой причине необходимо переопределить только `init()` метод без аргументов?
...
## 87. Поясните: Приведите пример использования собственных тегов.
...
## 88. Поясните: Различие между методами GET и POST
...
## 89. Поясните: Расскажите о методах жизненного цикла JSP.
...
## 90. Поясните: Расскажите об интерфейсе `SingleThreadModel`.
...
## 91. Поясните: Расскажите об этапах (фазах) жизненного цикла JSP.
...
## 92. Поясните: Статусы ответа HTTP
...
## 93. Поясните: Стоит ли волноваться о многопоточной безопасности работая с сервлетами?
...
## 94. Поясните: Часто встречающиеся коды ошибок HTTP
...
## 95. Поясните, чем SCRUM отличается от Kanban?
...
## 96. Поясните, чем отличается `<c:import>` от `<jsp:include>` и директивы `<%@include %>`?
...
## 97. Поясните, чем отличается `sendRedirect()` от `forward()`?
...
## 98. Поясните, чем отличаются _TCP_ и _UDP_?
...
## 99. Поясните, чем отличаются _Web server_ и _Application server_?
...
## 100. Поясните, чем отличаются методы _GET_ и _POST_?
...
## 101. Поясните: Что вы знаете о _сервлетных фильтрах_?
...
## 102. Поясните: Что вы знаете о `PageContext` и какие преимущества его использования?
...
## 103. Поясните: Что вы знаете о действиях JSP (_Action tag_ и _JSP Action Elements_).
...
## 104. Поясните: Что вы знаете о Языке выражений JSP (JSP Expression Language – EL)?
...
## 105. Поясните: Что означает _URL encoding_? Как это осуществить в Java?
...
## 106. Поясните: Что представляет собой `ServletConfig`?
...
## 107. Поясните: Что представляет собой `ServletContext`?
...
## 108. Поясните, что такое _AJAX_? Как принципиально устроена эта технология?
...
## 109. Поясните, что такое _Application server_?
...
## 110. Поясните, что такое _cookies_?
...
## 112. Поясните, что такое _Java Server Pages (JSP)_?
...
## 113. Поясните, что такое _JSON схема_?
...
## 115. Поясните, что такое _JSTL (JSP Standard tag library)_?
...
## 116. Поясните, что такое _MIME тип_?
...
## 117. Поясните, что такое _TCP/IP_?
...
## 119. Поясните, что такое _URL Rewriting_?
...
## 121. Поясните, что такое _Web application_?
...
## 122. Поясните, что такое _Web server_?
...
## 123. Поясните, что такое _WebSocket_?
...
## 125. Поясните, что такое _«авторизация»_ и _«аутентификация»_? Чем они отличаются?
...
## 126. Поясните, что такое _«дескриптор развертывания»_?
...
## 127. Поясните, что такое _«контейнер сервлетов»_?
...
## 128. Поясните, что такое _«сервлет»_?
...
## 129. Поясните, что такое _«сессия»_?
...
## 130. Поясните, что такое `Request Dispatcher`?
...
## 131. Поясните, что такое идемпотентный HTTP-метод?
...
## 132. Поясните, что такое протокол передачи данных? Какие протоколы вы знаете?
