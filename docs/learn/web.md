...
## 4. REST vs RPC. Как выбрать что лучше подходит?
...
## 10. Из чего состоит HTTP запрос
...
## 12. Из чего состоит JSON Web Token?
...
## 13. Назовите 6 принципов REST API
...
## 15. За что отвечают Cookie/куки?
...
## 16. Какие методы авторизации существуют для HTTP протокола?
...
## 17. Какие протоколы уровня приложений?
...
## 21. В каких ситуациях допустимо использование RESTless
...
## 22. В каком случае требуется переопределять метод `service()`?
...
## 23. В чем заключаются преимущества технологии сервлетов над CGI (Common Gateway Interface)?
...
## 25. В чем отличия `ServletContext` и `ServletConfig`?
...
## 26. В чем разница между `PrintWriter` и `ServletOutputStream`?
...
## 27. В чем разница между методами `GET` и `POST`?
...
## 28. Взаимодействие _JSP - сервлет - JSP_.
...
## 29. Всегда ли создается объект сессии на JSP странице, можно ли отключить его создание?
...
## 30. Для чего используются атрибуты сервлетов и как происходит работа с ними?
...
## 31. Для чего нужен интерфейс `ServletRequest`?
...
## 32. Для чего нужен интерфейс `ServletResponse`?
...
## 33. Есть ли смысл определять для сервлета конструктор? Каким образом лучше инициализировать данные?
...
## 34. Зачем в сервлетах используются различные _listener_?
...
## 36. Зачем нужны и чем отличаются методы `encodeURL()` и `encodeRedirectURL()`?
...
## 37. Зачем нужны сервера приложений, если есть контейнеры сервлетов?
...
## 39. Из каких групп тегов состоит библиотека _JSTL_?
...
## 40. Как закомментировать код в JSP?
...
## 41. Как из одного сервлета вызвать другой сервлет?
...
## 42. Как контейнер сервлетов управляет жизненным циклом сервлета, когда и какие методы вызываются?
...
## 43. Как конфигурируется JSP в дескрипторе развертывания.
...
## 44. Как можно обработать ошибки JSP страниц?
...
## 45. Как можно предотвратить прямой доступ к JSP странице из браузера?
...
## 46. Как мы можем обеспечить _transport layer security_ для нашего веб приложения?
...
## 48. Как организовать подключение к базе данных, обеспечить журналирование в сервлете?
...
## 49. Как отключить возможность использования EL в JSP?
...
## 50. Как получить IP адрес клиента на сервере?
...
## 51. Как получить информацию о сервере из сервлета?
...
## 52. Как получить реальное расположение сервлета на сервере?
...
## 53. Как происходит обработка ошибок с помощью JSTL?
...
## 54. Как реализовать запуск сервлета одновременно с запуском приложения?
...
## 55. Как сделать перенос строки в HTML средствами JSP?
...
## 56. Как сконфигурировать параметры инициализации для JSP?
...
## 57. Как уведомить объект в сессии, что сессия недействительна или закончилась?
...
## 58. Как узнать тип HTTP метода используя JSP EL?
...
## 59. Какая разница между _динамическим_ и _статическим_ содержимым JSP?
...
## 60. Какая разница между `<c:set>` и `<jsp:useBean>`?
...
## 61. Какая разница между `JSPWriter` и сервлетным `PrintWriter`?
...
## 62. Какие действия необходимо проделать при создании сервлетов?
...
## 63. Какие есть методы отправки данных с клиента на сервер?
...
## 64. Какие классы-обертки для сервлетов вы знаете?
...
## 65. Какие методы для работы с cookies предусмотрены в сервлетах?
...
## 66. Какие методы жизненного цикла JSP могут быть переопределены?
...
## 67. Какие наиболее распространенные задачи выполняются в контейнере сервлетов?
...
## 68. Какие неявные объекты не доступны в обычной JSP странице?
...
## 69. Какие неявные, внутренние объекты и методы есть на JSP странице?
...
## 70. Какие области видимости переменных существуют в JSP?
...
## 71. Какие основные методы присутствуют в классе `HttpServlet`?
...
## 72. Какие основные особенности появились в спецификации _Servlet 3_?
...
## 74. Какие способы аутентификации доступны сервлету?
...
## 75. Какие существуют основные типы тегов JSP?
...
## 76. Какие существуют уровни модели _OSI_?
...
## 77. Какие типы EL операторов вы знаете?
...
## 78. Каким образом можно допустить в сервлете deadlock?
...
## 79. Какова структура веб-проекта?
...
## 80. Какой метод HTTP не является неизменяемым?
...
## 81. Какой существует эффективный способ удостоверится, что все сервлеты доступны только для пользователя с верной сессией?
...
## 82. Когда стоит использовать фильтры сервлетов, а когда слушателей?
...
## 83. Можно ли использовать Javascript на JSP странице?
...
## 84. Можно ли одновременно использовать в сервлете `PrintWriter` и `ServletOutputStream`?
...
## 85. Можно ли определить класс внутри JSP страницы?
...
## 86. Назовите неявные, внутренние объекты JSP EL и их отличия от объектов JSP.
...
## 87. Опишите общие практические принципы работы с JSP.
...
## 88. Опишите, как обрабатываются JSP страницы, начиная от запроса к серверу, заканчивая ответом пользователю.
...
## 90. Почему `HttpServlet` класс объявлен как абстрактный?
...
## 91. Почему не нужно конфигурировать стандартные JSP теги в `web.xml`?
...
## 92. Почему не рекомендуется использовать скриплеты (скриптовые элементы) в JSP?
...
## 93. Почему необходимо переопределить только `init()` метод без аргументов?
...
## 94. Приведите пример использования собственных тегов.
...
## 95. Различие между методами GET и POST
...
## 96. Расскажите о методах жизненного цикла JSP.
...
## 97. Расскажите об интерфейсе `SingleThreadModel`.
...
## 98. Расскажите об этапах (фазах) жизненного цикла JSP.
...
## 100. Стоит ли волноваться о многопоточной безопасности работая с сервлетами?
...
## 101. Часто встречающиеся коды ошибок HTTP
...
## 102. Чем SCRUM отличается от Kanban?
...
## 103. Чем отличается `<c:import>` от `<jsp:include>` и директивы `<%@include %>`?
...
## 104. Чем отличается `sendRedirect()` от `forward()`?
...
## 105. Чем отличаются _TCP_ и _UDP_?
...
## 106. Чем отличаются _Web server_ и _Application server_?
...
## 107. Чем отличаются методы _GET_ и _POST_?
...
## 108. Что вы знаете о _сервлетных фильтрах_?
...
## 109. Что вы знаете о `PageContext` и какие преимущества его использования?
...
## 110. Что вы знаете о действиях JSP (_Action tag_ и _JSP Action Elements_).
...
## 111. Что вы знаете о Языке выражений JSP (JSP Expression Language – EL)?
...
## 112. Что означает _URL encoding_? Как это осуществить в Java?
...
## 113. Что представляет собой `ServletConfig`?
...
## 114. Что представляет собой `ServletContext`?
...
## 115. Что такое _AJAX_? Как принципиально устроена эта технология?
...
## 116. Что такое _Application server_?
...
## 119. Что такое _HTTP_ и _HTTPS_? Чем они отличаются?
...
## 120. Что такое _Java Server Pages (JSP)_?
...
## 123. Что такое _JSTL (JSP Standard tag library)_?
...
## 129. Что такое _Web application_?
...
## 133. Что такое _«авторизация»_ и _«аутентификация»_? Чем они отличаются?
...
## 134. Что такое _«дескриптор развертывания»_?
...
## 135. Что такое _«контейнер сервлетов»_?
...
## 138. Что такое `Request Dispatcher`?
...
## 139. Что такое идемпотентный HTTP-метод?
...
## 140. Что такое протокол передачи данных? Какие протоколы вы знаете?
