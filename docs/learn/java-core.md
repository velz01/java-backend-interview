...
## 2. Области памяти Java. Stack и heap. Что это такое?
...
## 3. Отличия интерфейса от абстрактных классов?
...
## 5. Какой класс называют Immutable?
...
## 6. Каков порядок вызова конструкторов и блоков инициализации с учетом иерархии классов?
...
## 7. Зачем нужны и какие бывают блоки инициализации?
...
## 8. Каким образом переменные передаются в методы — по значению или по ссылке?
...
## 9. В чем отличие между _поверхностным_ и _глубоким_ клонированием?
...
## 10. Возможно ли использование блока _try-finally_ (без `catch`)?
...
## 11. Для чего в Java используются статические блоки инициализации?
...
## 12. Для чего нужен метод `hashCode()`?
...
## 13. Есть класс `Point{int x, y;}`. Почему хэш код в виде `31 * x + y` предпочтительнее чем `x + y`?
...
## 14. Есть ли какие-либо рекомендации о том, какие поля следует использовать при подсчете `hashCode()`?
...
## 18. Каким образом реализованы методы hashCode() и equals() в классе Object?
...
## 19. Могут ли нестатические методы перегрузить статические?
...
## 20. Могут ли у разных объектов быть одинаковые `hashCode()`?
...
## 21. Может ли статический метод быть переопределён или перегружен?
...
## 22. Можно ли использовать строки в конструкции switch?
...
## 24. Почему метод `clone()` объявлен в классе `Object`, а не в интерфейсе `Cloneable`?
...
## 25. Почему строка является популярным ключом в `HashMap` в Java?
...
## 26. Расскажите про вложенные классы. В каких случаях они применяются?
...
## 27. Расскажите про клонирование объектов.
...
## 28. Что будет, если переопределить equals() не переопределяя hashCode()? Какие могут возникнуть проблемы?
...
## 29. Что делает метод `intern()` в классе `String`?.
...
## 30. Что такое _«конструктор по умолчанию»_?
...
## 31. Что такое _«локальный класс»_? Каковы его особенности?
...
## 32. Что такое «анонимные классы»? Где они применяются?
...
## 34. Что такое механизм _try-with-resources_?
...
## 35. Что такое сборщик мусора (garbage collector)?
...
## 36. Что такое пул строк/String pool?
...
## 38. Строки/String в Java? Из чего состоит? Нюансы
...
## 40. Разница между String, StringBuffer, StringBuilder
...
## 43. Переопределение и перегрузка методов
...
## 46. Какие типы ссылок существуют в Java?
...
## 49. Какие есть модификаторы доступа? Какие применимы к классам?
...
## 50. Расскажи что делает JIT компилятор
...
## 54. Что такое абстрактные классы?
...
## 58. Record-ы с Java 17. Для чего и зачем?
...
## 62. Что такое сериализация и для чего она нужна?
...
## 64. Автоупаковка и автораспаковка
...
## 65. За счет чего обеспечена кроссплатформенность?
...
## 66. Какие виды загрузчиков классов ты знаешь?
...
## 67. Может ли абстрактный класс быть final?
...
## 68. Можно ли сузить уровень доступа или тип возвращаемого значения при переопределении метода?
...
## 69. Почему не рекомендуется изменять строки в цикле?
...
## 70. Разница между версиями Java
...
## 72. Что можно изменить в сигнатуре метода при переопределении? Можно ли менять модификаторы (throws и т. п.)?
...
## 73. - Правила переопределения метода hashcode().
...
## 75. В `equals()` требуется проверять, что аргумент `equals(Object that)` такого же типа что и сам объект. В чем разница между `this.getClass() == that.getClass()` и `that instanceof MyClass`?
...
## 76. В чем разница между членом экземпляра класса и статическим членом класса?
...
## 77. Верно ли утверждение, что примитивные типы данных всегда хранятся в стеке, а экземпляры ссылочных типов данных в куче?
...
## 78. Возможно ли при переопределении метода изменить: модификатор доступа, возвращаемый тип, тип аргумента или их количество, имена аргументов или их порядок; убирать, добавлять, изменять порядок следования элементов секции `throws`?
...
## 79. Всегда ли исполняется блок `finally`?
...
## 80. Где и для чего используется модификатор `abstract`?
...
## 81. Где и как вы можете использовать приватный конструктор?
...
## 82. Где разрешена инициализация статических/нестатических полей?
...
## 83. Дайте определение понятию _«интерфейс»_. Какие модификаторы по умолчанию имеют поля и методы интерфейсов?
...
## 84. Дайте определение понятию «конструктор».
...
## 85. Для чего используется оператор `assert`?
...
## 86. Для чего нужен сборщик мусора?
...
## 87. Для чего нужна сериализация?
...
## 88. Если `equals()` переопределен, есть ли какие-либо другие методы, которые следует переопределить?
...
## 89. Если у класса `Point{int x, y;}` реализовать метод `equals(Object that) {(return this.x == that.x && this.y == that.y)}`, но сделать хэш код в виде `int hashCode() {return x;}`, то будут ли корректно такие точки помещаться и извлекаться из `HashSet`?
...
## 90. К каким конструкциям Java применим модификатор `static`?
...
## 91. Как написать собственное («пользовательское») исключение?
...
## 92. Как получить доступ к переопределенным методам родительского класса?
...
## 93. Как работает сборщик мусора?
...
## 94. Как создать Base64 кодировщик и декодировщик?
...
## 95. Как создать глубокую копию объекта? (2 способа)
...
## 96. Как создать собственный протокол сериализации?
...
## 98. Какая роль поля serialVersionUID в сериализации?
...
## 99. Какие виды классов есть в java?
...
## 100. Какие есть особенности класса `String`?
...
## 101. Какие логические операции и операторы вы знаете?
...
## 102. Какие методы есть у класса Object (перечислить все)? Что они делают?
...
## 103. Какие модификаторы доступа есть в Java? Какие применимы к классам?
...
## 105. Какие новые классы для работы с датами появились в java 8?
...
## 106. Какие нюансы у строк в Java?
...
## 107. Какие побитовые операции вы знаете?
...
## 109. Какие примитивные типы данных есть в Java?
...
## 110. Какие разновидности сборщиков мусора реализованы в виртуальной машине HotSpot?
...
## 111. Какие существуют _unchecked exception_?
...
## 112. Какие существуют модификаторы доступа?
...
## 113. Какие существуют особенности использования вложенных классов: статических и внутренних? В чем заключается разница между ними?
...
## 114. Какие терминальные методы в стримах вы знаете?
...
## 115. Какие типы классов бывают в java?
...
## 116. Каким образом передаются переменные в методы, по значению или по ссылке?
...
## 117. Какими значениями инициализируются переменные по умолчанию?
...
## 118. Какими значениями инициируются переменные по умолчанию?
...
## 119. Какими свойствами обладает порождаемое `equals()` отношение эквивалентности?
...
## 120. Какое исключение выбрасывается при возникновении ошибки в блоке инициализации класса?
...
## 121. Какой класс появился в Java 8 для кодирования/декодирования данных?
...
## 122. Какой контракт между hashCode() и equals()?
...
## 123. Какой оператор позволяет принудительно выбросить исключение?
...
## 124. Какой способ клонирования предпочтительней?
...
## 125. Когда в приложении может быть выброшено исключение `ClassCastException`?
...
## 126. Когда стоит изменять значение поля serialVersionUID?
...
## 127. Могут ли быть конструкторы у абстрактных классов? Для чего они нужны?
...
## 128. Могут ли быть приватные конструкторы? Для чего они нужны?
...
## 129. Могут ли у разных объектов `(ref0 != ref1)` быть `ref0.equals(ref1) == true`?
...
## 130. Могут ли у разных ссылок на один объект `(ref0 == ref1)` быть `ref0.equals(ref1) == false`?
...
## 131. Может ли быть абстрактный класс без абстрактных методов?
...
## 132. Может ли метод _main()_ выбросить исключение во вне и если да, то где будет происходить обработка данного исключения?
...
## 133. Может ли объект получить доступ к члену класса, объявленному как `private`? Если да, то каким образом?
...
## 134. Может ли один блок `catch` отлавливать сразу несколько исключений?
...
## 135. Может ли один интерфейс наследоваться от другого? От двух других?
...
## 136. Можно ли объявить метод абстрактным и статическим одновременно?
...
## 137. Можно ли реализовать метод `equals()` класса `MyClass` вот так: `class MyClass {public boolean equals(MyClass that) {return this == that;}}`?
...
## 138. Можно ли так реализовать метод `equals(Object that) {return this.hashCode() == that.hashCode()}`?
...
## 139. О чем говорит ключевое слово `final`?
...
## 140. О чем говорит ключевое слово `throws`?
...
## 141. Опишите алгоритм работы какого-нибудь сборщика мусора, реализованного в виртуальной машине HotSpot.
...
## 142. Опишите иерархию исключений.
...
## 143. Опишите процесс сериализации/десериализации с использованием Serializable.
...
## 144. Опишите работу блока _try-catch-finally_.
...
## 147. Почему `char[]` предпочтительнее `String` для хранения пароля?
...
## 148. Почему `String` неизменяемый и финализированный класс?
...
## 149. Почему в некоторых интерфейсах вообще не определяют методов?
...
## 150. Почему не рекомендуется изменять строки в цикле? Что рекомендуется использовать?
...
## 151. Почему нельзя объявить метод интерфейса с модификатором `final`?
...
## 152. Почему нельзя реализовать hashcode() который будет гарантированно уникальным для каждого объекта?
...
## 153. Правила переопределения equals()
...
## 154. Правила переопределения метода `Object.equals()`.
...
## 155. Предположим, есть метод, который может выбросить `IOException` и `FileNotFoundException` в какой последовательности должны идти блоки `catch`? Сколько блоков `catch` будет выполнено?
...
## 156. Расскажите про equals и hashcode
...
## 157. Расскажите про классы-загрузчики и про динамическую загрузку классов.
...
## 158. Расскажите про метод distinct()
...
## 159. Расскажите про метод filter()
...
## 160. Расскажите про метод limit()
...
## 161. Расскажите про метод reduce()
...
## 163. Расскажите про метод sorted()
...
## 164. Расскажите про приведение типов. Что такое понижение и повышение типа?
...
## 165. Существуют ли в java многомерные массивы?
...
## 166. Существуют ли ситуации, когда блок `finally` не будет выполнен?
...
## 167. Чем a.getClass().equals(A.class) отличается от a instanceOf A.class
...
## 168. Чем абстрактный класс отличается от интерфейса? В каких случаях следует использовать абстрактный класс, а в каких интерфейс?
...
## 169. Чем интерфейсы отличаются от абстрактных классов? В каких случаях следует использовать абстрактный класс, а в каких интерфейс?
...
## 170. Чем отличаются `final`, `finally` и `finalize()`?
...
## 171. Чем отличаются конструктор по-умолчанию, конструктор копирования и конструктор с параметрами?
...
## 172. Чем отличаются конструкторы по-умолчанию, конструктор копирования и конструктор с параметрами?
...
## 173. Чем различаются JRE, JVM и JDK?
...
## 174. Что вы знаете о `OutOfMemoryError`?
...
## 175. Что вы знаете о функции `main()`?
...
## 176. Что имеет более высокий уровень абстракции - _класс_, _абстрактный класс_ или _интерфейс_?
...
## 177. Что можно изменить в сигнатуре метода при переопределении? Можно ли менять модификаторы (throws и тп)?
...
## 178. Что означает модификатор static?
...
## 179. Что представляет из себя ошибки класса `Error`?
...
## 180. Что произойдет со сборщиком мусора, если выполнение метода `finalize()` требует ощутимо много времени, или в процессе выполнения будет выброшено исключение?
...
## 181. Что произойдёт, если в блоке инициализации возникнет исключительная ситуация?
...
## 182. Что такое _autoboxing («автоупаковка»)_ в Java и каковы правила упаковки примитивных типов в классы-обертки?
...
## 183. Что такое _checked_ и _unchecked exception_?
...
## 185. Что такое _Heap_ и _Stack_ память в Java? Какая разница между ними?
...
## 187. Что такое _«интернационализация»_, _«локализация»_?
...
## 188. Что такое _«статический класс»_?
...
## 189. Что такое `finalize()`? Зачем он нужен?
...
## 195. Что такое абстрактные классы? Чем они отличаются от обычных?
...
## 196. Что такое автоупаковка и автораспаковка?
...
## 197. Что такое интерфейсы? Какие модификаторы по умолчанию имеют поля и методы интерфейсов?
...
## 198. Что такое класс `Object`? Какие в нем есть методы?
...
## 202. Что такое перечисления (enum)?
...
## 203. Что такое сериализация и как она реализована в Java?
...
## 204. Что такое тернарный оператор выбора?
