...
Кратко: Идемпотентность — это свойство операции, при котором повторный вызов даёт тот же результат, что и первый.
...
## 3. Зачем нужен Reflection?

<div class="answer-block">

Кратко: Reflection в Java — это мощный механизм, позволяющий «заглянуть» внутрь классов и объектов во время выполнения программы, а также динамически создавать экземпляры, вызывать методы и получать/изменять значения полей без знания их имён на этапе компиляции.
...
## 4. Поясните: Определение сложности алгоритма
...
## 5. Поясните: Обычное программирование vs реактивщина
...
## 6. Поясните: Инструменты для отладки приложения, течет память, все замирает - Visual VM, JMap, профилировщики
...
## 7. Каким образом выявлять и устранять узкие места производительности в REST-сервисах и микросервисных архитектурах, когда рост нагрузки (например, увеличение трафика или обращений к БД) приводит к значительно более долгой обработке запросов, чем ожидается, и какие стратегии оптимизации (в том числе архитектурные изменения и масштабирование) могут помочь достичь требуемых временных рамок отклика?
...
## 8. Поясните: Какой подход к неймингу методов позволяет ясно и однозначно отражать их функциональность, например, для методов, которые выполняют поиск с сортировкой, очистку кэша или расчёт с сохранением данных и возвратом статуса?
...
## 9. Поясните: Code first vs Design first

<div class="answer-block">

Кратко: Code First — сначала код, потом всё остальное. Сначала у нас считается логика, запускаются функции и нажимаются кнопочки, а уже потом придумывается интерфейс.
...
## 10. Поясните: В каких пакетах расположены классы потоков ввода/вывода?

<div class="answer-block">

Кратко: `java.io`, `java.nio`. Для работы с потоками компрессированных данных используются классы из пакета `java.util.zip`
...
## 11. Поясните, в чём отличие класса `PrintWriter` от `PrintStream`?
...
## 12. Поясните, в чём заключается разница между IO и NIO?
...
## 13. Поясните: Вопросы на проработку легенды

<div class="answer-block">

Кратко: Как происходил процесс автоматизации версий и выхода в продакшн? Как заменяли версии?
...
## 14. Зачем используется `PushbackInputStream`?

<div class="answer-block">

Кратко: Разновидность буферизации, обеспечивающая чтение байта с последующим его возвратом в поток. Класс `PushbackInputStream` представляет механизм «заглянуть» во входной поток и увидеть, что оттуда поступит в следующий момент, не извлекая информации.
...
## 15. Зачем используется `SequenceInputStream`?
...
## 16. Каким образом выбрать все элементы определенного каталога по критерию (например, с определенным расширением)?
...
## 17. Какие именно классы поддерживают чтение и запись потоков в компрессированном формате?
...
## 18. Какие именно классы позволяют преобразовать байтовые потоки в символьные и обратно?
...
## 19. Какие именно классы позволяют ускорить чтение/запись за счет использования буфера?
...
## 20. Какие именно методы класса `File` вы знаете?

<div class="answer-block">

Кратко: Наиболее используемые методы класса `File`:
...
## 21. Какие именно особенности NIO вы знаете?
...
## 22. Какие именно подклассы класса `InputStream` вы знаете, для чего они предназначены?
...
## 23. Какие именно подклассы класса `OutputStream` вы знаете, для чего они предназначены?
...
## 24. Какие именно подклассы класса `Reader` вы знаете, для чего они предназначены?
...
## 25. Какие именно подклассы класса `Writer` вы знаете, для чего они предназначены?
...
## 26. Какие именно режимы доступа к файлу есть у `RandomAccessFile`?
...
## 27. Поясните: Какой класс позволяет читать данные из входного байтового потока в формате примитивных типов данных?
...
## 28. Поясните: Какой класс предназначен для работы с элементами файловой системы?
...
## 29. Поясните: Какой символ является разделителем при указании пути в файловой системе?

<div class="answer-block">

Кратко: Для различных операционных систем символ разделителя различается. Для Windows это `\`, для Linux - `/`. 
...
## 31. Перечислите основные классы потоков ввода/вывода.

<div class="answer-block">

Кратко: Разделяют два вида потоков ввода/вывода: 
...
## 32. Поясните: Существует ли возможность перенаправить потоки стандартного ввода/вывода?
...
## 33. Поясните, чем unit тесты отличаются от интеграционных?

<div class="answer-block">

Кратко: Из институтского курса по технологиям программирования я вынес следующую классификацию видов тестирования (критерий — степень изолированности кода). Тестирование бывает:
...
## 34. Поясните, чем отличаются и что общего у `InputStream`, `OutputStream`, `Reader`, `Writer`?
...
## 35. Поясните: Что вы знаете о `RandomAccessFile`?
...
## 36. Поясните: Что вы знаете об интерфейсе `FileFilter`?
...
## 37. Поясните, что такое _«абсолютный путь»_ и _«относительный путь»_?

<div class="answer-block">

Кратко: __Абсолютный (полный) путь__ — это путь, который указывает на одно и то же место в файловой системе, вне зависимости от текущей рабочей директории или других обстоятельств. Полный путь всегда начинается с корневого каталога. 
...
## 38. Поясните, что такое _«каналы»_?
...
## 39. Поясните, что такое _«символьная ссылка»_?
