...
## 1. Поясните, чем отличается процесс от потока?
...
## 3. Поясните: Какими способами можем запустить несколько потоков в Java приложении? Как запустить поток в Java?
...
## 4. Поясните: Каким образом можно создать поток?
...
## 5. Поясните: Что за интерфейсы Runnable и Callable? В чем между ними отличия?
...
## 6. Поясните, чем Thread отличается от Runnable? Когда нужно использовать Thread, а когда Runnable? (Ответ что тред - это класс, а ранбл интерфейс - считается не полным, нужно рассказать подробно)
...
## 7. Поясните: В каких состояниях может находиться поток?
...
## 8. Поясните, в чём заключается разница между методами `start()` и `run()`?
...
## 9. Поясните, что такое потоки демоны? Для чего они нужны? Как создать поток-демон?
...
## 10. Допустимо ли сделать основной поток программы демоном?
...
## 11. Каким образом остановить поток?
...
## 12. Каким образом правильно остановить поток? Для чего нужны методы .stop(), .interrupt(), .interrupted(), .isInterrupted().
...
## 13. Поясните: Что значит _«приоритет потока»_?
...
## 14. Поясните, что такое приоритет потока? На что он влияет? Какой приоритет у потоков по умолчанию?
...
## 15. Поясните, что такое синхронизация? Какие способы синхронизации существуют в Java?
...
## 16. Поясните: Напишите простейший многопоточный ограниченный буфер с использованием `synchronized`.
...
## 17. Зачем используется ключевое слово `volatile`, `synchronized`, `transient`, `native`?
...
## 18. Каким образом проверить, удерживает ли поток монитор определённого ресурса?
...
## 19. Допустимо ли создавать новые экземпляры класса, пока выполняется `static synchronized` метод?
...
## 20. Поясните: На каком объекте происходит синхронизация при вызове `static synchronized` метода?
...
## 21. Перечислите различия между `synchronized` и `ReentrantLock`?
...
## 22. Что представляет собой монитор и как он реализован в Java?
...
## 23. Поясните: Что является монитором у нестатического synchronized-метода?
...
## 24. Поясните: Что является монитором у статического synchronized-метода?
...
## 25. Поясните: Collections.synchronizedMap vs ConcurrentHashMap
...
## 26. Поясните, в чём разница между `notify()` и `notifyAll()`?
...
## 27. Как устроена работа методов wait(), notify() и notifyAll()?
...
## 28. По какой причине методы `wait()` и `notify()` вызываются только в синхронизированном блоке?
...
## 29. Поясните, чем отличается работа метода `wait()` с параметром и без параметра?
...
## 30. Поясните, чем отличаются методы wait() и sleep()?
...
## 33. Поясните: Atomic vs Volatile. Что это и когда использовать?
...
## 34. Поясните, в чём заключаются различия между `java.util.concurrent.Atomic*.compareAndSwap()` и `java.util.concurrent.Atomic*.weakCompareAndSwap()`.
...
## 35. Поясните, в чём различия между `volatile` и _Atomic_ переменными?
...
## 36. Зачем нужны Atomic типы данных? Чем отличаются от volatile?
...
## 38. Поясните, что такое _ordering_, _as-if-serial semantics_, _sequential consistency_, _visibility_, _atomicity_, _happens-before_, _mutual exclusion_, _safe publication_?
...
## 39. Что такое deadlock и как его избежать?
...
## 41. Поясните: Напишите простейший многопоточный ограниченный буфер с использованием `ReentrantLock`.
...
## 42. Поясните: Напишите минимальный неблокирующий стек (всего два метода — `push()` и `pop()`) с использованием `Semaphore`.
...
## 43. Поясните, что такое `ReadWriteLock`?
...
## 44. Поясните, что такое `Semaphore`?
...
## 45. Поясните, что такое _double checked locking Singleton_?
...
## 46. Поясните, что такое FutureTask?
...
## 47. Поясните, что такое ExecutorService?
...
## 48. Поясните, что такое CompletableFuture?
...
## 49. Поясните: ForkJoinPool vs FixedThreadPool
...
## 50. Поясните: Stream API &amp; ForkJoinPool. Как связаны, что это такое.
...
## 51. Поясните, что такое _«фреймворк Fork/Join»_?
...
## 52. Поясните: Executors.newCachedThreadPool()
...
## 53. Поясните: Какого размера должен быть пул потоков?
...
## 54. Поясните, что такое _«пул потоков»_?
...
## 55. Поясните, что такое happens-before?
...
## 57. Поясните: Существует ли способ решения проблемы _race condition_?
...
## 58. Расскажи подробнее про пакет concurrent
...
## 59. Поясните: util. Concurrent поверхностно.
...
## 60. Поясните, что такое _ThreadLocal-переменная_?
...
## 61. Поясните, что такое CAS операции? (Compare And Swap)
...
## 62. Каким образом работать с коллекциями в многопоточном программировании?
...
## 63. Каким образом работает Thread.join()? Для чего он нужен?
...
## 64. Поясните, в чём разница между `interrupted()` и `isInterrupted()`?
...
## 65. Каким образом создать потокобезопасный Singleton?
...
## 66. Поясните, что такое «потокобезопасность»?
...
## 67. Поясните: Асинхронность vs многопоточность
...
## 68. Поясните: Области памяти в многопоточности?
...
## 69. Поясните: Способы решения задач для многопоточного доступа
...
## 70. Поясните, в чём заключаются различия между cтеком (stack) и кучей (heap) с точки зрения многопоточности?
...
## 71. Назовите принципы, которым вы следуете в многопоточном программировании?
...
## 72. Допустимо ли вызвать start() для одного потока дважды?
...
## 73. Для чего нужны пулы потоков?
...
## 74. Поясните, чем конкурентность отличается от параллелизма?
...
## 75. Поясните, в чём заключаются различия между `CyclicBarrier` и `CountDownLatch`?
...
## 76. Поясните, в чём заключается различие между методами `submit()` и `execute()` у пула потоков?
...
## 77. Поясните, в чём разница между _«конкуренцией»_ и _«параллелизмом»_?
...
## 78. Для чего может быть нужен `private` мьютекс?
...
## 79. Каким образом поделиться данными между двумя потоками?
...
## 80. Каким образом получить дамп потока?
...
## 82. Поясните: Какое из следующих утверждений о потоках неверно?
...
## 83. Поясните: Какой параметр запуска JVM используется для контроля размера стека потока?
...
## 84. Поясните: Напишите минимальный неблокирующий ArrayList (всего четыре метода — `add()`, `get()`, `remove()`, `size()`).
...
## 85. Поясните: Напишите минимальный неблокирующий стек (всего два метода — `push()` и `pop()`).
...
## 86. По какой причине не рекомендуется использовать метод `Thread.stop()`?
...
## 87. Поясните: Расскажите о модели памяти Java?
...
## 88. Поясните, чем полезны неизменяемые объекты?
...
## 90. Поясните: Что значит _«усыпить»_ поток?
...
## 91. Поясните: Что происходит, когда в потоке выбрасывается исключение?
...
## 92. Поясните, что такое _busy spin_?
...
## 93. Поясните, что такое _«блокирующий метод»_?
...
## 94. Поясните, что такое _«зелёные потоки»_ и есть ли они в Java?
...
## 95. Поясните, что такое _«кооперативная многозадачность»_? Какой тип многозадачности использует Java? Чем обусловлен этот выбор?
...
## 96. Поясните, что такое семафор? Как он реализован в Java?
