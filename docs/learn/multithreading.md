...
- **Видео:** [Многопоточность в Java: основы — Computer Science Center](https://www.youtube.com/watch?v=zxZ0BXlTys0)
...
## 1. Поясните, чем процесс отличается от потока?
...
## 2. Поясните, что такое многопоточность?
...
## 3. Поясните: Каким образом можно создать поток?
...
## 4. Поясните: В каких состояниях может находиться поток?
...
## 5. Поясните, что такое потоки демоны? Для чего они нужны? Как создать поток-демон?
...
## 6. Поясните: Напишите простейший многопоточный ограниченный буфер с использованием `synchronized`.
...
## 7. Поясните: Collections.synchronizedMap vs ConcurrentHashMap
...
## 8. Поясните: SynchronizedCollections. Что это? За счет чего достигается потокобезопасность?
...
## 9. Зачем используется ключевое слово `volatile`, `synchronized`, `transient`, `native`?
...
## 10. Каким образом проверить, удерживает ли поток монитор определённого ресурса?
...
## 11. Допустимо ли создавать новые экземпляры класса, пока выполняется `static synchronized` метод?
...
## 12. Поясните: На каком объекте происходит синхронизация при вызове `static synchronized` метода?
...
## 13. Перечислите различия между `synchronized` и `ReentrantLock`?
...
## 14. Что представляет собой монитор и как он реализован в Java?
...
## 15. Поясните: Что является монитором у нестатического synchronized-метода?
...
## 16. Поясните: Что является монитором у статического synchronized-метода?
...
## 17. Поясните: ForkJoinPool vs FixedThreadPool
...
## 18. Поясните: Stream API &amp; ForkJoinPool. Как связаны, что это такое.
...
## 19. Поясните, в чём разница между `notify()` и `notifyAll()`?
...
## 20. Каким образом работает Thread.join()? Для чего он нужен?
...
## 21. Как устроена работа методов wait(), notify() и notifyAll()?
...
## 22. По какой причине методы `wait()` и `notify()` вызываются только в синхронизированном блоке?
...
## 23. Поясните, чем отличается работа метода `wait()` с параметром и без параметра?
...
## 24. Поясните, чем отличаются методы `Thread.sleep()` и `Thread.yield()`?
...
## 25. Поясните, чем отличаются методы wait() и sleep()?
...
## 26. Поясните, что такое _«фреймворк Fork/Join»_?
...
## 27. Поясните, что такое Deadlock
...
## 28. Поясните, что такое Livelock
...
## 31. Поясните: Atomic vs Volatile. Что это и когда использовать?
...
## 32. Поясните: Напишите простейший многопоточный ограниченный буфер с использованием `ReentrantLock`.
...
## 33. Поясните, в чём заключаются различия между `java.util.concurrent.Atomic*.compareAndSwap()` и `java.util.concurrent.Atomic*.weakCompareAndSwap()`.
...
## 34. Поясните, в чём различия между `volatile` и _Atomic_ переменными?
...
## 35. Зачем нужны Atomic типы данных? Чем отличаются от volatile?
...
## 36. Поясните: Напишите минимальный неблокирующий стек (всего два метода — `push()` и `pop()`) с использованием `Semaphore`.
...
## 37. Поясните: Что означает ключевое слово volatile? Почему операции над volatile переменными не атомарны?
...
## 38. Поясните, что такое _double checked locking Singleton_?
...
## 39. Поясните, что такое _ordering_, _as-if-serial semantics_, _sequential consistency_, _visibility_, _atomicity_, _happens-before_, _mutual exclusion_, _safe publication_?
...
## 40. Поясните, что такое `ReadWriteLock`?
...
## 41. Поясните, что такое `Semaphore`?
...
## 42. Поясните, что такое FutureTask?
...
## 43. Поясните, что такое ExecutorService?
...
## 44. Поясните, что такое CompletableFuture?
...
## 45. Поясните: Executors.newCachedThreadPool()
...
## 46. Поясните, что такое Race condition
...
## 47. Поясните, что такое happens-before?
...
## 48. Поясните: Существует ли способ решения проблемы _race condition_?
...
## 49. Каким образом создать потокобезопасный Singleton?
...
## 50. Поясните, что такое синхронизация? Какие способы синхронизации существуют в Java?
...
## 51. Каким образом работать с коллекциями в многопоточном программировании?
...
## 52. Поясните: Какими способами можем запустить несколько потоков в Java приложении? Как запустить поток в Java?
...
## 53. Поясните: Что за интерфейсы Runnable и Callable? В чем между ними отличия?
...
## 54. Поясните, что такое CAS операции? (Compare And Swap)
...
## 55. Поясните: Асинхронность vs многопоточность
...
## 56. Поясните: Области памяти в многопоточности?
...
## 57. Поясните: Способы решения задач для многопоточного доступа
...
## 58. Поясните, в чём заключаются различия между cтеком (stack) и кучей (heap) с точки зрения многопоточности?
...
## 59. Назовите принципы, которым вы следуете в многопоточном программировании?
...
## 60. Поясните, что такое «потокобезопасность»?
...
## 61. Поясните, в чём заключается разница между методами `start()` и `run()`?
...
## 62. Допустимо ли вызвать start() для одного потока дважды?
...
## 63. Расскажи подробнее про пакет concurrent
...
## 64. Поясните: Что значит потокобезопасность?
...
## 65. Для чего нужны пулы потоков?
...
## 66. Поясните, чем Thread отличается от Runnable? Когда использовать Thread, а когда Runnable?
...
## 67. Поясните, чем конкурентность отличается от параллелизма?
...
## 69. Поясните: util. Concurrent поверхностно.
...
## 70. Поясните, в чём заключаются различия между `CyclicBarrier` и `CountDownLatch`?
...
## 71. Поясните, в чём разница между `interrupted()` и `isInterrupted()`?
...
## 72. Поясните, в чём заключается различие между методами `submit()` и `execute()` у пула потоков?
...
## 73. Поясните, в чём разница между _«конкуренцией»_ и _«параллелизмом»_?
...
## 74. Поясните: Дайте определение понятию «синхронизация».
...
## 75. Поясните: Даны 3 потока Т1, Т2 и Т3? Как реализовать выполнение в последовательности Т1, Т2, Т3?
...
## 76. Для чего может быть нужен `private` мьютекс?
...
## 77. Каким образом остановить поток?
...
## 78. Каким образом поделиться данными между двумя потоками?
...
## 79. Каким образом получить дамп потока?
...
## 80. Каким образом правильно остановить поток? Для чего нужны методы .stop(), .interrupt(), .interrupted(), .isInterrupted().
...
## 81. Каким образом принудительно запустить поток?
...
## 82. Какие именно существуют способы синхронизации в Java?
...
## 83. Поясните: Какого размера должен быть пул потоков?
...
## 84. Поясните: Какое из следующих утверждений о потоках неверно?
...
## 85. Поясните: Какой параметр запуска JVM используется для контроля размера стека потока?
...
## 86. Допустимо ли сделать основной поток программы демоном?
...
## 87. Поясните: Напишите минимальный неблокирующий ArrayList (всего четыре метода — `add()`, `get()`, `remove()`, `size()`).
...
## 88. Поясните: Напишите минимальный неблокирующий стек (всего два метода — `push()` и `pop()`).
...
## 89. Поясните: Напишите потокобезопасную реализацию класса с неблокирующим методом `BigInteger next()`, который возвращает элементы последовательности: `[1, 2, 4, 8, 16, ...]`.
...
## 90. По какой причине не рекомендуется использовать метод `Thread.stop()`?
...
## 91. Поясните: Расскажите о модели памяти Java?
...
## 92. Поясните, чем Runnable отличается от Callable?
...
## 93. Поясните, чем Thread отличается от Runnable? Когда нужно использовать Thread, а когда Runnable? (Ответ что тред - это класс, а ранбл интерфейс - считается не полным, нужно рассказать подробно)
...
## 94. Поясните, чем отличается процесс от потока?
...
## 95. Поясните, чем отличаются два интерфейса `Runnable` и `Callable`?
...
## 96. Поясните, чем полезны неизменяемые объекты?
...
## 97. Поясните, чем различаются `Thread` и `Runnable`?
...
## 98. Поясните: Что будет, если очередь пула потоков уже заполнена, но подаётся новая задача?
...
## 99. Поясните: Что значит _«приоритет потока»_?
...
## 100. Поясните: Что значит _«усыпить»_ поток?
...
## 101. Поясните: Что происходит, когда в потоке выбрасывается исключение?
...
## 102. Поясните, что такое _busy spin_?
...
## 103. Поясните, что такое _ThreadLocal-переменная_?
...
## 104. Поясните, что такое _«блокирующий метод»_?
...
## 105. Поясните, что такое _«зелёные потоки»_ и есть ли они в Java?
...
## 106. Поясните, что такое _«кооперативная многозадачность»_? Какой тип многозадачности использует Java? Чем обусловлен этот выбор?
...
## 107. Поясните, что такое _«пул потоков»_?
...
## 108. Поясните, что такое приоритет потока? На что он влияет? Какой приоритет у потоков по умолчанию?
...
## 109. Поясните, что такое семафор? Как он реализован в Java?
