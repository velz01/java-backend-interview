...
## 8. Чем процесс отличается от потока?
...
## 10. Какими способами можем запустить несколько потоков в Java приложении? Как запустить поток в Java?
...
## 12. Что такое CompletableFuture?
...
## 13. Как работать с коллекциями в многопоточном программировании?
...
## 17. Что такое CAS операции? (Compare And Swap)
...
## 19. Асинхронность vs многопоточность
...
## 22. Области памяти в многопоточности?
...
## 23. Что такое монитор? Как реализован в Java?
...
## 24. Расскажи про пакет concurrent
...
## 25. Какие бывают состояния у потока?
...
## 27. ForkJoinPool vs FixedThreadPool
...
## 29. Способы решения задач для многопоточного доступа
...
## 30. Чем Thread отличается от Runnable? Когда использовать Thread, а когда Runnable?
...
## 31. Чем конкурентность отличается от параллелизма?
...
## 33. Stream API &amp; ForkJoinPool. Как связаны, что это такое.
...
## 34. SynchronizedCollections. Что это? За счет чего достигается потокобезопасность?
...
## 35. util. Concurrent поверхностно.
...
## 36. В чем заключаются различия между `CyclicBarrier` и `CountDownLatch`?
...
## 37. В чем заключаются различия между cтеком (stack) и кучей (heap) с точки зрения многопоточности?
...
## 38. В чем разница между `interrupted()` и `isInterrupted()`?
...
## 39. В чем разница между `notify()` и `notifyAll()`?
...
## 40. В чём заключается различие между методами `submit()` и `execute()` у пула потоков?
...
## 41. В чём заключается разница между методами `start()` и `run()`?
...
## 42. В чём заключаются различия между `java.util.concurrent.Atomic*.compareAndSwap()` и `java.util.concurrent.Atomic*.weakCompareAndSwap()`.
...
## 43. В чём различия между `volatile` и _Atomic_ переменными?
...
## 44. В чём разница между _«конкуренцией»_ и _«параллелизмом»_?
...
## 45. Дайте определение понятию «синхронизация».
...
## 46. Даны 3 потока Т1, Т2 и Т3? Как реализовать выполнение в последовательности Т1, Т2, Т3?
...
## 47. Для чего используется ключевое слово `volatile`, `synchronized`, `transient`, `native`?
...
## 48. Для чего нужны Atomic типы данных? Чем отличаются от volatile?
...
## 49. Зачем может быть нужен `private` мьютекс?
...
## 51. Как поделиться данными между двумя потоками?
...
## 53. Как правильно остановить поток? Для чего нужны методы .stop(), .interrupt(), .interrupted(), .isInterrupted().
...
## 54. Как принудительно запустить поток?
...
## 55. Как проверить, удерживает ли поток монитор определённого ресурса?
...
## 56. Как работает Thread.join()? Для чего он нужен?
...
## 57. Как работает метод `Thread.join()`?
...
## 58. Как работают методы `wait()` и `notify()`/`notifyAll()`?
...
## 59. Как работают методы wait(), notify() и notifyAll()?
...
## 60. Как создать потокобезопасный Singleton?
...
## 61. Какие существуют способы синхронизации в Java?
...
## 62. Каким образом можно создать поток?
...
## 63. Какого размера должен быть пул потоков?
...
## 64. Какое из следующих утверждений о потоках неверно?
...
## 65. Какой параметр запуска JVM используется для контроля размера стека потока?
...
## 66. Можно ли вызвать start() для одного потока дважды?
...
## 67. Можно ли сделать основной поток программы демоном?
...
## 68. Можно ли создавать новые экземпляры класса, пока выполняется `static synchronized` метод?
...
## 69. На каком объекте происходит синхронизация при вызове `static synchronized` метода?
...
## 70. Назовите различия между `synchronized` и `ReentrantLock`?
...
## 71. Напишите минимальный неблокирующий ArrayList (всего четыре метода — `add()`, `get()`, `remove()`, `size()`).
...
## 72. Напишите минимальный неблокирующий стек (всего два метода — `push()` и `pop()`) с использованием `Semaphore`.
...
## 73. Напишите минимальный неблокирующий стек (всего два метода — `push()` и `pop()`).
...
## 74. Напишите потокобезопасную реализацию класса с неблокирующим методом `BigInteger next()`, который возвращает элементы последовательности: `[1, 2, 4, 8, 16, ...]`.
...
## 75. Напишите простейший многопоточный ограниченный буфер с использованием `ReentrantLock`.
...
## 76. Напишите простейший многопоточный ограниченный буфер с использованием `synchronized`.
...
## 77. Перечислите принципы, которым вы следуете в многопоточном программировании?
...
## 78. Почему методы `wait()` и `notify()` вызываются только в синхронизированном блоке?
...
## 79. Почему не рекомендуется использовать метод `Thread.stop()`?
...
## 80. Расскажите о модели памяти Java?
...
## 81. Существует ли способ решения проблемы _race condition_?
...
## 82. Чем Runnable отличается от Callable?
...
## 83. Чем Thread отличается от Runnable? Когда нужно использовать Thread, а когда Runnable? (Ответ что тред - это класс, а ранбл интерфейс - считается не полным, нужно рассказать подробно)
...
## 84. Чем отличается процесс от потока?
...
## 85. Чем отличается работа метода `wait()` с параметром и без параметра?
...
## 86. Чем отличаются два интерфейса `Runnable` и `Callable`?
...
## 87. Чем отличаются методы `Thread.sleep()` и `Thread.yield()`?
...
## 88. Чем отличаются методы wait() и sleep()?
...
## 89. Чем полезны неизменяемые объекты?
...
## 90. Чем различаются `Thread` и `Runnable`?
...
## 91. Что будет, если очередь пула потоков уже заполнена, но подаётся новая задача?
...
## 92. Что значит _«приоритет потока»_?
...
## 93. Что значит _«усыпить»_ поток?
...
## 94. Что означает ключевое слово synchronized? Где и для чего может использоваться?
...
## 95. Что означает ключевое слово volatile? Почему операции над volatile переменными не атомарны?
...
## 96. Что происходит, когда в потоке выбрасывается исключение?
...
## 98. Что такое _double checked locking Singleton_?
...
## 99. Что такое _ordering_, _as-if-serial semantics_, _sequential consistency_, _visibility_, _atomicity_, _happens-before_, _mutual exclusion_, _safe publication_?
...
## 100. Что такое _ThreadLocal-переменная_?
...
## 101. Что такое _«блокирующий метод»_?
...
## 102. Что такое _«зелёные потоки»_ и есть ли они в Java?
...
## 103. Что такое _«кооперативная многозадачность»_? Какой тип многозадачности использует Java? Чем обусловлен этот выбор?
...
## 104. Что такое _«монитор»_ в Java?
...
## 105. Что такое _«потоки-демоны»_?
...
## 107. Что такое _«фреймворк Fork/Join»_?
...
## 110. Что такое «потокобезопасность»?
...
## 111. Что такое монитор? Как монитор реализован в java?
...
## 112. Что такое потоки демоны? Для чего они нужны? Как создать поток-демон?
...
## 113. Что такое приоритет потока? На что он влияет? Какой приоритет у потоков по умолчанию?
...
## 114. Что такое семафор? Как он реализован в Java?
...
## 115. Что такое Фреймворк fork/join? Для чего он нужен?
...
## 116. Что является монитором у нестатического synchronized-метода?
...
## 117. Что является монитором у статического synchronized-метода?
