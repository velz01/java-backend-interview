...
Кратко: Да, будет, но в этом случае HashMap вырождается в связный список и теряет своим преимущества
...
Кратко: 1. Вычисляем хеш-код ключа и бакет, в который будет добавлен новый элемент
...
Кратко: Queue - это интерфейс в Java, который представляет собой коллекцию элементов, работающую по принципу “первый вошел - первый вышел” (FIFO — First In, First Out)
...
Кратко: `TreeSet` обеспечивает упорядоченно хранение элементов в виде красно-черного дерева. Сложность выполнения основных операций не хуже _O(log(N))_ (_Логарифмическое время_). 
...
Кратко: Это возможно в случае, если метод, определяющий номер корзины будет возвращать одинаковые значения.
...
Кратко: Интерфейс Iterable имеет метод - iterator(), с типом возвращаемого значения - интерфейс Iterator.
...
Кратко: Если вызову `iterator.remove()` предшествовал вызов `iterator.next()`, то `iterator.remove()` удалит элемент коллекции, на который указывает итератор, в противном случае будет выброшено `IllegalStateException()`.
...
Кратко: ___O(N)___. Худший случай - это поиск ключа в `HashMap`, вырожденного в список по причине совпадения ключей по `hashCode()` и для выяснения хранится ли элемент с определённым ключом может потребоваться перебор всего списка.
...
Кратко: Каждый элемент `LinkedList` хранит ссылку на предыдущий элемент, следующий элемент и ссылку на данные.
...
Кратко: Коллекция (List и Set) представляет собой совокупность некоторых элементов (обычно экземпляров одного класса).Map -это совокупность пар "ключ"-"значение".
...
Кратко: В основе `TreeSet` лежит красно-черное дерево, которое умеет само себя балансировать. В итоге, `TreeSet` все равно в каком порядке вы добавляете в него элементы, преимущества этой структуры данных будут сохраняться.
...
Кратко: _«Коллекция»_ - это структура данных, набор каких-либо объектов. Данными (объектами в наборе) могут быть числа, строки, объекты пользовательских классов и т.п.
...
Кратко: Это список, реализованный на основе динамически расширяемого массива. То есть под капотом буквально создается массив
...
Кратко: HashSet - коллекция, не содержащая в себе дубликатов, реализация интерфейса Set, под капотом HashMap с заглушкой вместо значения
...
Кратко: Интерфейс `Map` выделен отдельно, так как он не является коллекцией в привычном смысле. Его предназначение - хранить пары “ключ-значение”, а не просто элементы.
...
Кратко: TreeSet - коллекция, хранящая элементы в отсортированном порядке, дубликаты не допускаются (перезатираются). Под капотом TreeMap - связь аналогична HashMap и HashSet: вместо значений используется заглушка. Поиск за O(log n), можно передать компаратор в конструктор.
...
Кратко: В LinkedList элементы фактически представляют собой звенья одной цепи. У каждого элемента помимо тех данных, которые он хранит, имеется ссылка на предыдущий и следующий элемент. По этим ссылкам можно переходить от одного элемента к другому.
...
Кратко: У LinkedList есть внутренний статический класс `Node`, символизирующий элемент списка, а также поля `first` и `last`, в которых хранятся первый и последний элементы (узлы/ноды). У каждого элемент есть ссылка на предыдущий элемент (`prev`) и следующий (`next`). При добавлении элемента ссылки переприсваиваются.
...
Кратко: Древовидная структура: под капотом TreeMap использует структуру данных, которая называется **красно-чёрное дерево**:
...
Кратко: LinkedHashMap - это структура данных, представляющая собой HashMap, но пары ключ-значение образуют двусвязный список. При создании по умолчанию хранит пары в порядке вставки, но в конструкторе можно задать хранение в порядке доступа, т.е. может работать как LRU кэш.
...
Кратко: LinkedList - это связный список. Каждый элемент такого списка хранит помимо значения ещё и ссылки на одного из своих соседей, либо на обоих в зависимости от типа списка - односвязный или двусвязный. LinkedList в Java - это двусвязный список.
...
Кратко: О(log n), т.к каждый шаг прохода отметает половину.
...
Кратко: HashSet использует HashMap под капотом: объекты являются ключами, а вместо значений используется константа-заглушка. Поэтому алгоритм добавления идентичен HashMap (элемент HashSet == ключ HashMap)
...
Кратко: Да, будет, но его производительность ухудшится. Если все элементы имеют одинаковый хеш-код, они все будут размещены в одном бакете, что приведет к увеличению времени поиска
...
Кратко: HashSet использует HashMap для хранения элементов
...
Кратко: `TreeSet` — хранит элементы в отсортированном порядке (по естественному порядку или по предоставленному компаратору) и гарантирует их уникальность.
...
Кратко: Deque (Double-Ended Queue) - это интерфейс в Java, представляющий собой двунаправленную очередь (”дек”). Это означает, что элементы могут быть добавлены или удалены как с начала, так и с конца очереди. Таким образом, Deque может работать как очередь (**FIFO**) и как стек (**LIFO**).
...
Кратко: `Двусвязный`: каждый элемент `LinkedList` хранит ссылку на предыдущий и следующий элементы.
...
Кратко: PhantomReference при вызове метода `get()` возвращает всегда `null`, поэтому тяжело представить назначение такой структуры данных.
...
Кратко: Так же оно проявляется при итерации по коллекции.
...
## 66. Поясните, в чём разница между классами `java.util.Collection` и `java.util.Collections`?

<div class="answer-block">

Кратко: `java.util.Collections` - набор статических методов для работы с коллекциями.
...
## 67. Поясните: Для `ArrayList` или для `LinkedList` операция добавления элемента в середину (`list.add(list.size()/2, newElement)`) медленнее?

<div class="answer-block">

Кратко: Для `ArrayList`:
...
## 68. Поясните: Для `Enum` есть специальный класс `java.util.EnumSet`. Зачем? Чем авторов не устраивал `HashSet` или `TreeSet`?
...
## 69. Для чего добавили `ArrayList`, если уже был `Vector`?
...
## 70. Для чего нужен `HashMap`, если есть `Hashtable`?
...
## 71. Каким образом и когда происходит увеличение количества корзин в `HashMap`?

<div class="answer-block">

Кратко: Помимо `capacity` у `HashMap` есть еще поле `loadFactor`, на основании которого, вычисляется предельное количество занятых корзин `capacity * loadFactor`. По умолчанию `loadFactor = 0.75`. По достижению предельного значения, число корзин увеличивается в 2 раза и для всех хранимых элементов вычисляется новое «местоположение» с учетом нового числа корзин.
...
## 72. Каким образом из коллекции создать стрим?

<div class="answer-block">

Кратко: Collection&lt;String&gt; collection = Arrays.asList("a1", "a2", "a3");
...
## 73. Каким образом избежать `ConcurrentModificationException` во время перебора коллекции?
...
## 74. Каким образом между собой связаны `Iterable` и `Iterator`?

<div class="answer-block">

Кратко: Интерфейс `Iterable` имеет только один метод - `iterator()`, который возвращает `Iterator`.
...
## 75. Каким образом одним вызовом из `List` получить `List` со всеми элементами, кроме первых и последних 3-х?

<div class="answer-block">

```java
Кратко: List<Integer> subList = list.subList(3, list.size() - 3);
```
...
## 76. Каким образом одной строчкой преобразовать `ArrayList` в `HashSet`?

<div class="answer-block">

```java
Кратко: HashSet<Integer> set = new HashSet<>(new ArrayList<>());
```
...
## 77. Каким образом одной строчкой преобразовать `HashSet` в `ArrayList`?

<div class="answer-block">

```java
Кратко: ArrayList<Integer> list = new ArrayList<>(new HashSet<>());
```
...
## 78. Каким образом одной строчкой скопировать элементы любой `collection` в массив?

<div class="answer-block">

```java
Кратко: Object[] array = collection.toArray();
```
...
## 79. Каким образом перебрать все значения `Map`?

<div class="answer-block">

Кратко: Использовать метод `values()`, который возвращает коллекцию `Collection<V>` значений.
...
## 80. Каким образом перебрать все ключи `Map`?

<div class="answer-block">

Кратко: Использовать метод `keySet()`, который возвращает множество `Set<K>` ключей.
...
## 81. Каким образом перебрать все пары «ключ-значение» в `Map`?

<div class="answer-block">

Кратко: Использовать метод `entrySet()`, который возвращает множество `Set<Map.Entry<K, V>>` пар «ключ-значение».
...
## 82. Каким образом перебрать элементы `LinkedList` в обратном порядке, не используя медленный `get(index)`?

<div class="answer-block">

Кратко: Для этого в `LinkedList` есть обратный итератор, который можно получить вызва метод `descendingIterator()`.
...
## 83. Каким образом поведёт себя уже инстанциированный итератор для `collection`, если вызвать `collection.remove()`?

<div class="answer-block">

Кратко: При следующем вызове методов итератора будет выброшено `ConcurrentModificationException`.
...
## 84. Каким образом получить коллекцию только для чтения?
...
## 85. Каким образом происходит удаление элементов из `ArrayList`? Как меняется в этом случае размер `ArrayList`?

<div class="answer-block">

Кратко: При удалении произвольного элемента из списка, все элементы, находящиеся «правее» смещаются на одну ячейку влево и реальный размер массива (его емкость, capacity) не изменяется никак. Механизм автоматического «расширения» массива существует, а вот автоматического «сжатия» нет, можно только явно выполнить «сжатие» командой `trimToSize()`.
...
## 86. Каким образом устроен `HashMap`?
...
## 87. Каким образом устроен ArrayList, сложность основных операций.

<div class="answer-block">

Кратко: ArrayList реализован внутри в виде обычного массива. Поэтому при вставке элемента в середину, приходится сначала сдвигать на один все элементы после него, а уже затем в освободившееся место вставлять новый элемент. 
...
## 88. Каким образом устроен HashSet, сложность основных операций.

<div class="answer-block">

Кратко: Все классы, реализующие интерфейс Set, внутренне поддерживаются реализациями Map. HashSet хранит элементы с помощью HashMap. Значение, которые мы передаем в HashSet, является ключом к объекту HashMap, а в качестве значения используется Object.
...
## 89. В чём отличие LinkedHashSet от HashSet?

<div class="answer-block">

Кратко: `LinkedHashSet` отличается от `HashSet` только тем, что в его основе лежит `LinkedHashMap` вместо `HashMap`. Благодаря этому порядок элементов при обходе коллекции является идентичным порядку добавления элементов (insertion-order). При добавлении элемента, который уже присутствует в `LinkedHashSet` (т.е. с одинаковым ключом), порядок обхода элементов не изменяется.
...
## 90. Каким образом устроен TreeSet, сложность основных операций.

<div class="answer-block">

Кратко: Время для базовых операций - Логарифмическое время.
...
## 91. Каким образом устроена HashMap, сложность основных операций? (Расскажите про принцип корзин)
...
## 92. Каким образом устроена TreeMap, сложность основных операций?

<div class="answer-block">

Кратко: Класс TreeMap&lt;K, V&gt; представляет отображение в виде дерева. Он наследуется от класса AbstractMap и реализует интерфейс NavigableMap, а следовательно, также и интерфейс SortedMap. Поэтому в отличие от коллекции HashMap в TreeMap все объекты автоматически сортируются по возрастанию их ключей.
...
## 93. Поясните: Как, используя LinkedHashMap, сделать кэш c «invalidation policy»?
...
## 94. Поясните: Какая разница между fail-fast и fail-safe?

<div class="answer-block">

Кратко: В противоположность fail-fast, итераторы fail-safe не вызывают никаких исключений при изменении структуры, потому что они работают с клоном коллекции вместо оригинала.
...
## 95. Поясните: Какая разница между итераторами с fail-fast и fail-safe поведением? (С примерами)

<div class="answer-block">

Кратко: Итератор fail-safe не вызывает исключений при изменении структуры коллекции, потому что работает с её клоном.
...
## 96. Какие именно нововведения появились в java 8?

<div class="answer-block">

Кратко: 1. Полноценная поддержка лямбда-вражений
2. Ссылки на методы ::
...
## 97. Какие именно промежуточные методы в стримах вы знаете?

<div class="answer-block">

Кратко: filter(boolean - Predicate)
map()
flatMap()
limit(n)
skip(n)
...
## 98. Какие именно существуют реализации Map?

<div class="answer-block">

Кратко: TreeMap, HashMap, HashTable, LinkedHashMap
...
## 99. Какие именно существуют способы перебирать элементы списка?
...
## 100. Поясните: Каким образом можно получить синхронизированные объекты стандартных коллекций?

<div class="answer-block">

Кратко: С помощью статических методов `synchronizedMap()` и `synchronizedList()` класса `Collections`. Данные методы возвращают синхронизированный декоратор переданной коллекции. При этом все равно в случае обхода по коллекции требуется ручная синхронизация. 
...
## 101. Поясните: Какова оценка временной сложности операций над элементами из `HashMap`? Гарантирует ли `HashMap` указанную сложность выборки элемента?

<div class="answer-block">

Кратко: В общем случае операции добавления, поиска и удаления элементов занимают константное время. 
...
## 102. Поясните: Какова роль `equals()` и `hashCode()` в `HashMap`?
...
## 103. Поясните: Каково максимальное число значений `hashCode()`?

<div class="answer-block">

Кратко: Число значений следует из сигнатуры `int hashCode()` и равно диапазону типа `int` — __2&lt;sup&gt;32&lt;/sup&gt;__.
...
## 104. Поясните: Какое начальное количество корзин в `HashMap`?

<div class="answer-block">

Кратко: В конструкторе по умолчанию - 16, используя конструкторы с параметрами можно задавать произвольное начальное количество корзин.
...
## 105. Поясните: Какое худшее время работы метода `add()` для `ArrayList`?

<div class="answer-block">

Кратко: _O(N)_. Вставка элемента в конец списка осуществляется за время _O(1)_, но если вместимость массива недостаточна, то происходит создание нового массива с увеличенным размером и копирование всех элементов из старого массива в новый.
...
## 106. Поясните: Какое худшее время работы метода `add()` для `LinkedList`?

<div class="answer-block">

Кратко: _O(N)_. Добавление в начало/конец списка осуществляется за время _O(1)_.
...
## 107. Поясните: Какое худшее время работы метода `contains()` для элемента, который есть в `ArrayList`?

<div class="answer-block">

Кратко: _O(N)_. Время поиска элемента линейно пропорционально количеству элементов с списке.
...
## 108. Поясните: Какое худшее время работы метода `contains()` для элемента, который есть в `LinkedList`?

<div class="answer-block">

Кратко: _O(N)_. Время поиска элемента линейно пропорционально количеству элементов в списке.
...
## 109. Поясните: Какое худшее время работы метода get(key) для ключа, которого нет в HashMap?

<div class="answer-block">

Кратко: O(N). Худший случай - это поиск ключа в таблице, вырожденной в список, перебор ключей которой занимает линейно пропорциональное время количеству хранимых элементов.
...
## 110. Поясните: Кто кого расширяет: `Queue` расширяет `Deque`, или `Deque` расширяет `Queue`?

<div class="answer-block">

Кратко: `Queue` - это очередь, которая обычно (но необязательно) строится по принципу FIFO (First-In-First-Out) - соответственно извлечение элемента осуществляется с начала очереди, вставка элемента - в конец очереди. Хотя этот принцип нарушает, к примеру, `PriorityQueue`, использующая «natural ordering» или переданный `Comparator` при вставке нового элемента.
...
## 111. Допустимо ли итерируясь по ArrayList удалить элемент? Какое вылетит исключение?

<div class="answer-block">

Кратко: Можно, но нужно использовать iterator.remove().
...
## 112. Перечислите главные реализации Deque
...
## 113. Перечислите основные интерфейсы JCF и их реализации.
...
## 114. Поясните: Напишите однопоточную программу, которая заставляет коллекцию выбросить `ConcurrentModificationException`.

<div class="answer-block">

```java
Кратко: public static void main(String[] args) {
...
## 115. Поясните: Необходимо добавить 1 млн. элементов, какую структуру вы используете?

<div class="answer-block">

Кратко: Однозначный ответ можно дать только исходя из информации о том в какую часть списка происходит добавление элементов, что потом будет происходить с элементами списка, существуют ли какие-то ограничения по памяти или скорости выполнения.
...
## 116. Поясните смысл параметров в конструкторе `HashMap(int initialCapacity, float loadFactor)`.
...
## 117. По какой причине `LinkedList` реализует и `List`, и `Deque`?
...
## 118. По какой причине несмотря на то, что ключ в `HashMap` не обязан реализовывать интерфейс `Comparable`, двусвязный список всегда удается преобразовать в красно-черное дерево?

<div class="answer-block">

Кратко: Красно-черное дерево - это самобалансирующееся бинарное дерево поиска. Это означает, что для его построения нужно уметь сравнивать элементы между собой. 
...
## 120. Поясните: Приведите пример реализации Deque.

<div class="answer-block">

Кратко: Linked list, Stack, ArrayDeque
...
## 121. Поясните: Приведите пример, когда какая-либо коллекция выбрасывает `UnsupportedOperationException`.
...
## 122. Поясните: Приведите примеры итераторов, реализующих поведение fail-safe

<div class="answer-block">

Кратко: Итератор коллекции `CopyOnWriteArrayList` и итератор представления `keySet` коллекции `ConcurrentHashMap` являются примерами итераторов fail-safe.
...
## 123. Поясните: Расположите в виде иерархии следующие интерфейсы: `List`, `Set`, `Map`, `SortedSet`, `SortedMap`, `Collection`, `Iterable`, `Iterator`, `NavigableSet`, `NavigableMap`.
...
## 124. Расскажите подробнее про интерфейс List

<div class="answer-block">

Кратко: Контейнеры List хранит элементы в порядке добавления. Интерфейс List дополняет Collection несколькими методами, обеспечивающими вставку и удаление элементов в середине списка.
...
## 125. Расскажите подробнее про интерфейс Set.

<div class="answer-block">

Кратко: Интерфейс Set расширяет интерфейс Collection. 
...
## 126. Расскажите подробнее про метод collect()

<div class="answer-block">

Stream.collect () является одним из терминальных методов. Это даёт возможность выполнять изменяемые операции свертывания (переупаковка элементов в некоторые структуры данных и применение некоторой дополнительной логики, объединение их и т. Д.)
...
## 127. Расскажите подробнее про метод flatMap().

<div class="answer-block">

Кратко: flatMap возвращает по стриму для каждого объекта в первоначальном стриме, а затем результирующие потоки объединяются в исходный стрим.
...
## 128. Расскажите подробнее про метод map().

<div class="answer-block">

Кратко: Метод map() заданным образом преобразует каждый элемент стрима, потом преобразует все объекты в итоговый стрим.
...
## 130. Расскажите подробнее про реализации интерфейса Set

<div class="answer-block">

Кратко: В HashSet порядок добавления элементов будет непредсказуемым - используется хэширование для ускорения выборки.
...
## 131. Поясните: Реализуйте симметрическую разность двух коллекций используя методы `Collection` (`addAll(...)`, `removeAll(...)`, `retainAll(...)`).

<div class="answer-block">

Кратко: Симметрическая разность двух коллекций - это множество элементов, одновременно не принадлежащих обоим исходным коллекциям.
...
## 132. Поясните: Сделайте `HashMap` из `HashSet<Map.Entry<K, V>>`.

<div class="answer-block">

```java
Кратко: HashMap<K, V> map = new HashMap<>(set.size());
...
## 133. Поясните: Сделайте `HashSet` из ключей `HashMap`.

<div class="answer-block">

```java
Кратко: HashSet<Object> set = new HashSet<>(map.keySet());
```
...
## 134. Поясните: Сколько выделяется дополнительно памяти при вызове `LinkedList.add()`?

<div class="answer-block">

Кратко: Создается один новый экземпляр вложенного класса `Node`.
...
## 135. Поясните: Сколько необходимо дополнительной памяти при вызове `ArrayList.add()`?

<div class="answer-block">

Кратко: Если в массиве достаточно места для размещения нового элемента, то дополнительной памяти не требуется. Иначе происходит создание нового массива размером в 1,5 раза превышающим существующий (это верно для JDK выше 1.7, в более ранних версиях размер увеличения иной).
...
## 136. Поясните: Сколько переходов происходит в момент вызова `HashMap.get(key)` по ключу, который есть в таблице?
...
## 137. Поясните: Сколько создается новых объектов, когда вы добавляете новый элемент в `HashMap`?

<div class="answer-block">

Кратко: __Один__ новый объект статического вложенного класса `Entry<K,V>`.
...
## 138. Поясните: Сколько элементов будет пропущено, если `Iterator.next()` будет вызван после 10-ти вызовов `Iterator.hasNext()`?

<div class="answer-block">

Кратко: Нисколько - `hasNext()` осуществляет только проверку наличия следующего элемента.
...
## 139. Поясните: Согласно Кнуту и Кормену существует две основных реализации хэш-таблицы: на основе открытой адресации и на основе метода цепочек. Как реализована `HashMap`? Почему, по вашему мнению, была выбрана именно эта реализация? В чем плюсы и минусы каждого подхода?
...
## 140. Поясните: Сравните `Iterator` и `ListIterator`.
...
## 141. Поясните, чем Set отличается от List?

<div class="answer-block">

Кратко: Set не добавляет новых методов, только вносит изменения унаследованные.
В частности, метод add() добавляет элемент в коллекцию и возвращает true, если не было такого элемента.
Разрешено наличие только одной ссылки типа null.
...
## 142. Поясните: Что будет, если мы кладем в HashMap ключ, у которого equals и hashCode определены некорректно?

<div class="answer-block">

Кратко: Объект скорее всего добавится, но обратно мы не сможем получить его.
...
## 143. Поясните: Что позволяет сделать `PriorityQueue`?
...
## 144. Поясните: Что произойдет при вызове `Iterator.next()` без предварительного вызова `Iterator.hasNext()`?

<div class="answer-block">

Кратко: Если итератор указывает на последний элемент коллекции, то возникнет исключение `NoSuchElementException`, иначе будет возвращен следующий элемент.
...
## 145. В чём различия между ArrayList и LinkedList?
...
## 146. Поясните, что такое «fail-fast поведение»?
