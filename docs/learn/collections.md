...
- **Видео:** [Полный гайд по Java HashMap](https://www.youtube.com/watch?v=D6r4AbMElx0)

---

**• Низкий приоритет**

## 1. Перечислите основные интерфейсы JCF и их реализации.
...
## 3. Поясните, что такое ArrayList?
...
## 5. Поясните, что такое TreeSet? Внутреннее устройство
...
## 6. Поясните, что такое LinkedList?
...
## 7. Что такое `HashMap` и как она устроена?
...
## 8. Поясните: Процесс добавления объекта в HashMap
...
## 9. Поясните: Возможна ли ситуация, когда `HashMap` выродится в список даже с ключами имеющими разные `hashCode()`?
...
## 10. Каким образом работает `HashMap` при попытке сохранить в него два элемента по ключам с одинаковым `hashCode()`, но для которых `equals() == false`?
...
## 11. Поясните: Какова роль `equals()` и `hashCode()` в `HashMap`?
...
## 12. Поясните: Что будет, если мы кладем в HashMap ключ, у которого equals и hashCode определены некорректно?
...
## 13. Поясните: HashMap - сложность операций
...
## 14. Поясните: Какое худшее время работы метода get(key) для ключа, который есть в `HashMap`?
...
## 15. По какой причине нельзя использовать `byte[]` в качестве ключа в `HashMap`?
...
## 17. Поясните: HashMap vs TreeMap
...
## 18. Каким образом HashSet связан с HashMap?
...
## 19. Поясните: `LinkedHashMap` - что в нем от `LinkedList`, а что от `HashMap`?
...
## 20. Поясните: Сделайте `HashMap` из `HashSet<Map.Entry<K, V>>`.
...
## 22. Поясните: Могу положить в HashMap или TreeMap элемент с ключом null?
...
## 23. Поясните: В `WeakHashMap` используются WeakReferences. А почему бы не создать `PhantomHashMap` на PhantomReferences?
...
## 24. Поясните: В `WeakHashMap` используются WeakReferences. А почему бы не создать `SoftHashMap` на SoftReferences?
...
## 25. Поясните: В каком случае может быть потерян элемент в `HashMap`?
...
## 26. Поясните, в чём разница между `HashMap` и `IdentityHashMap`? Для чего нужна `IdentityHashMap`?
...
## 27. Поясните, в чём разница между `HashMap` и `WeakHashMap`? Для чего используется `WeakHashMap`?
...
## 28. Для чего нужен `HashMap`, если есть `Hashtable`?
...
## 29. Каким образом и когда происходит увеличение количества корзин в `HashMap`?
...
## 30. Каким образом устроена HashMap, сложность основных операций? (Расскажите про принцип корзин)
...
## 31. Поясните: Как, используя LinkedHashMap, сделать кэш c «invalidation policy»?
...
## 32. Поясните: Какова оценка временной сложности операций над элементами из `HashMap`? Гарантирует ли `HashMap` указанную сложность выборки элемента?
...
## 33. Поясните: Какое начальное количество корзин в `HashMap`?
...
## 34. Поясните: Какое худшее время работы метода get(key) для ключа, которого нет в HashMap?
...
## 35. Поясните смысл параметров в конструкторе `HashMap(int initialCapacity, float loadFactor)`.
...
## 36. По какой причине несмотря на то, что ключ в `HashMap` не обязан реализовывать интерфейс `Comparable`, двусвязный список всегда удается преобразовать в красно-черное дерево?
...
## 37. Поясните: Сколько переходов происходит в момент вызова `HashMap.get(key)` по ключу, который есть в таблице?
...
## 38. Поясните: Сколько создается новых объектов, когда вы добавляете новый элемент в `HashMap`?
...
## 39. Поясните: Согласно Кнуту и Кормену существует две основных реализации хэш-таблицы: на основе открытой адресации и на основе метода цепочек. Как реализована `HashMap`? Почему, по вашему мнению, была выбрана именно эта реализация? В чем плюсы и минусы каждого подхода?
...
## 40. Поясните: Внутреннее устройство TreeMap
...
## 42. Поясните: TreeMap - сложность операций
...
## 43. Каким образом устроена TreeMap, сложность основных операций?
...
## 44. Каким образом между собой связаны Iterable, Iterator и «for-each»?
...
## 45. Каким образом поведёт себя коллекция, если вызвать `iterator.remove()`?
...
## 46. Поясните, чем различаются Enumeration и Iterator?
...
## 47. Каким образом с собой связаны Iterable и foreach?
...
## 48. Каким образом между собой связаны `Iterable` и `Iterator`?
...
## 49. Поясните: Сколько элементов будет пропущено, если `Iterator.next()` будет вызван после 10-ти вызовов `Iterator.hasNext()`?
...
## 50. Поясните: Сравните `Iterator` и `ListIterator`.
...
## 51. Поясните: Что произойдет при вызове `Iterator.next()` без предварительного вызова `Iterator.hasNext()`?
...
## 52. Поясните, в чём отличия `TreeSet` и `HashSet`?
...
## 53. Поясните: Что будет, если добавлять элементы в `TreeSet` по возрастанию?
...
## 54. Поясните: ArrayList. Что происходит под капотом при добавлении/удалении элемента в начало/середину/конец списка?
...
## 55. Поясните: Какая скорость вставки в ArrayList и LinkedList
...
## 56. Поясните: LinkedList. Что происходит под капотом при добавлении/удалении элемента в начало/середину/конец списка?
...
## 57. Поясните, что такое бакет? Внутреннее устройство бакета
...
## 58. Поясните: Оцените количество памяти на хранение одного примитива типа `byte` в `ArrayList`?
...
## 59. Поясните: Оцените количество памяти на хранение одного примитива типа `byte` в `LinkedList`?
...
## 60. По какой причине Map — это не Collection, в то время как List и Set являются Collection?
...
## 61. Поясните, что такое _«коллекция»_?
...
## 62. Поясните: ArrayList - сложность операций
...
## 63. Поясните: Реализации интерфейса Map. Почему интерфейс Map выделен отдельно?
...
## 64. Поясните: LinkedList - сложность операций
...
## 65. Поясните: Внутреннее устройство HashSet
...
## 66. Поясните: Процесс добавления объекта в HashSet?
...
## 67. Поясните: Будет ли работать HashSet, если все добавляемые элементы будут иметь одинаковый hashCode()?
...
## 68. Поясните: ArrayList. Какая размерность массива по умолчанию?
...
## 69. Поясните: `LinkedList` — это односвязный, двусвязный или четырехсвязный список?
...
## 70. Поясните: В реализации класса `ArrayList` есть следующие поля: `Object[] elementData`, `int size`. Объясните, зачем хранить отдельно `size`, если всегда можно взять `elementData.length`?
...
## 71. Поясните: Для `ArrayList` или для `LinkedList` операция добавления элемента в середину (`list.add(list.size()/2, newElement)`) медленнее?
...
## 72. Поясните: Для `Enum` есть специальный класс `java.util.EnumSet`. Зачем? Чем авторов не устраивал `HashSet` или `TreeSet`?
...
## 73. Для чего добавили `ArrayList`, если уже был `Vector`?
...
## 74. Каким образом перебрать элементы `LinkedList` в обратном порядке, не используя медленный `get(index)`?
...
## 75. Каким образом происходит удаление элементов из `ArrayList`? Как меняется в этом случае размер `ArrayList`?
...
## 76. В чём отличие LinkedHashSet от HashSet?
...
## 77. Допустимо ли итерируясь по ArrayList удалить элемент? Какое вылетит исключение?
...
## 78. По какой причине `LinkedList` реализует и `List`, и `Deque`?
...
## 79. Поясните: Предложите эффективный алгоритм удаления нескольких рядом стоящих элементов из середины списка, реализуемого `ArrayList`.
...
## 80. В чём различия между ArrayList и LinkedList?
...
## 85. Поясните: Структуры данных. Какая сложность в дереве?
...
## 86. Поясните: HashSet - сложность операций
...
## 87. В чём отличие Deque от Queue?
...
## 89. Поясните: HashTable - что это такое?
...
## 90. Поясните: Какую коллекцию использовать для уникальных отсортированных значений?
...
## 91. Перечислите главные реализации Queue
...
## 93. Поясните: `Stack` считается «устаревшим». Чем его рекомендуют заменять? Почему?
...
## 94. Поясните: Deque - сложность операций
...
## 95. Поясните: Queue - сложность операций
...
## 96. Поясните, в чём проявляется «сортированность» `SortedMap`, кроме того, что `toString()` выводит все элементы по порядку?
...
## 97. Поясните, в чём разница между java.util.Collection и java.util.Collections?
...
## 98. Каким образом избежать `ConcurrentModificationException` во время перебора коллекции?
...
## 99. Каким образом поведёт себя уже инстанциированный итератор для `collection`, если вызвать `collection.remove()`?
...
## 100. Каким образом получить коллекцию только для чтения?
...
## 101. Каким образом устроен ArrayList, сложность основных операций.
...
## 102. Каким образом устроен HashSet, сложность основных операций.
...
## 103. Каким образом устроен TreeSet, сложность основных операций.
...
## 104. Какие именно нововведения появились в java 8?
...
## 105. Какие именно существуют реализации Map?
...
## 106. Какие именно существуют способы перебирать элементы списка?
...
## 107. Поясните: Каким образом можно получить синхронизированные объекты стандартных коллекций?
...
## 108. Поясните: Каково максимальное число значений `hashCode()`?
...
## 109. Поясните: Какое худшее время работы метода `add()` для `ArrayList`?
...
## 110. Поясните: Какое худшее время работы метода `add()` для `LinkedList`?
...
## 111. Поясните: Какое худшее время работы метода `contains()` для элемента, который есть в `ArrayList`?
...
## 112. Поясните: Какое худшее время работы метода `contains()` для элемента, который есть в `LinkedList`?
...
## 113. Поясните: Кто кого расширяет: `Queue` расширяет `Deque`, или `Deque` расширяет `Queue`?
...
## 116. Поясните: Приведите пример, когда какая-либо коллекция выбрасывает `UnsupportedOperationException`.
...
## 117. Расскажите подробнее про интерфейс List
...
## 118. Расскажите подробнее про интерфейс Set.
...
## 119. Расскажите подробнее про реализации интерфейса Set
...
## 120. Поясните: Реализуйте симметрическую разность двух коллекций используя методы `Collection` (`addAll(...)`, `removeAll(...)`, `retainAll(...)`).
...
## 121. Поясните: Сколько выделяется дополнительно памяти при вызове `LinkedList.add()`?
...
## 122. Поясните: Сколько необходимо дополнительной памяти при вызове `ArrayList.add()`?
...
## 123. Поясните, чем Set отличается от List?
...
## 124. Поясните: Что позволяет сделать `PriorityQueue`?
