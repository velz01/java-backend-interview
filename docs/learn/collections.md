...
## 1. Будет ли работать HashMap, если все добавляемые ключи будут иметь одинаковый hashCode()?
...
## 2. Расскажите как выглядит иерархия коллекций
...
## 4. Внутреннее устройство HashMap
...
## 5. Процесс добавления объекта в HashMap
...
## 8. В чем отличия `TreeSet` и `HashSet`?
...
## 9. Возможна ли ситуация, когда `HashMap` выродится в список даже с ключами имеющими разные `hashCode()`?
...
## 10. Как между собой связаны Iterable, Iterator и «for-each»?
...
## 12. Как работает `HashMap` при попытке сохранить в него два элемента по ключам с одинаковым `hashCode()`, но для которых `equals() == false`?
...
## 13. Какое худшее время работы метода get(key) для ключа, который есть в `HashMap`?
...
## 14. Оцените количество памяти на хранение одного примитива типа `byte` в `ArrayList`?
...
## 15. Оцените количество памяти на хранение одного примитива типа `byte` в `LinkedList`?
...
## 16. Почему Map — это не Collection, в то время как List и Set являются Collection?
...
## 17. Почему нельзя использовать `byte[]` в качестве ключа в `HashMap`?
...
## 19. Чем различаются Enumeration и Iterator?
...
## 20. Что будет, если добавлять элементы в `TreeSet` по возрастанию?
...
## 24. Что такое бакет? Внутреннее устройство бакета
...
## 25. ArrayList. Что происходит под капотом при добавлении/удалении элемента в начало/середину/конец списка?
...
## 27. ArrayList - сложность операций
...
## 29. Реализации интерфейса Map. Почему интерфейс Map выделен отдельно?
...
## 31. Какая скорость вставки в ArrayList и LinkedList
...
## 32. Что такое TreeSet? Внутреннее устройство
...
## 33. LinkedList - сложность операций
...
## 34. LinkedList. Что происходит под капотом при добавлении/удалении элемента в начало/середину/конец списка?
...
## 35. Внутреннее устройство TreeMap
...
## 38. Внутреннее устройство HashSet
...
## 39. Могу положить в HashMap или TreeMap элемент с ключом null?
...
## 41. Как с собой связаны Iterable и foreach?
...
## 45. Какая коллекция реализует дисциплину обслуживания LIFO? FIFO?
...
## 48. Процесс добавления объекта в HashSet?
...
## 49. Будет ли работать HashSet, если все добавляемые элементы будут иметь одинаковый hashCode()?
...
## 50. Как HashSet связан с HashMap?
...
## 51. ArrayList. Какая размерность массива по умолчанию?
...
## 52. Какую коллекцию использовать для уникальных отсортированных значений?
...
## 53. Назовите главные реализации Queue
...
## 55. `LinkedHashMap` - что в нем от `LinkedList`, а что от `HashMap`?
...
## 56. `LinkedList` — это односвязный, двусвязный или четырехсвязный список?
...
## 57. `Stack` считается «устаревшим». Чем его рекомендуют заменять? Почему?
...
## 60. В `WeakHashMap` используются WeakReferences. А почему бы не создать `PhantomHashMap` на PhantomReferences?
...
## 61. В `WeakHashMap` используются WeakReferences. А почему бы не создать `SoftHashMap` на SoftReferences?
...
## 62. В каком случае может быть потерян элемент в `HashMap`?
...
## 63. В реализации класса `ArrayList` есть следующие поля: `Object[] elementData`, `int size`. Объясните, зачем хранить отдельно `size`, если всегда можно взять `elementData.length`?
...
## 64. В чем проявляется «сортированность» `SortedMap`, кроме того, что `toString()` выводит все элементы по порядку?
...
## 65. В чем разница между `HashMap` и `IdentityHashMap`? Для чего нужна `IdentityHashMap`?
...
## 66. В чем разница между `HashMap` и `WeakHashMap`? Для чего используется `WeakHashMap`?
...
## 67. В чем разница между java.util.Collection и java.util.Collections?
...
## 68. В чем разница между классами `java.util.Collection` и `java.util.Collections`?
...
## 69. Для `ArrayList` или для `LinkedList` операция добавления элемента в середину (`list.add(list.size()/2, newElement)`) медленнее?
...
## 70. Для `Enum` есть специальный класс `java.util.EnumSet`. Зачем? Чем авторов не устраивал `HashSet` или `TreeSet`?
...
## 71. Зачем добавили `ArrayList`, если уже был `Vector`?
...
## 72. Зачем нужен `HashMap`, если есть `Hashtable`?
...
## 73. Как и когда происходит увеличение количества корзин в `HashMap`?
...
## 74. Как из коллекции создать стрим?
...
## 75. Как избежать `ConcurrentModificationException` во время перебора коллекции?
...
## 76. Как между собой связаны `Iterable` и `Iterator`?
...
## 77. Как одним вызовом из `List` получить `List` со всеми элементами, кроме первых и последних 3-х?
...
## 78. Как одной строчкой преобразовать `ArrayList` в `HashSet`?
...
## 79. Как одной строчкой преобразовать `HashSet` в `ArrayList`?
...
## 81. Как перебрать все значения `Map`?
...
## 82. Как перебрать все ключи `Map`?
...
## 83. Как перебрать все пары «ключ-значение» в `Map`?
...
## 84. Как перебрать элементы `LinkedList` в обратном порядке, не используя медленный `get(index)`?
...
## 85. Как поведёт себя уже инстанциированный итератор для `collection`, если вызвать `collection.remove()`?
...
## 86. Как получить коллекцию только для чтения?
...
## 87. Как происходит удаление элементов из `ArrayList`? Как меняется в этом случае размер `ArrayList`?
...
## 89. Как устроен ArrayList, сложность основных операций.
...
## 90. Как устроен HashSet, сложность основных операций.
...
## 91. Как устроен LinkedHashSet, сложность основных операций.
...
## 92. Как устроен TreeSet, сложность основных операций.
...
## 93. Как устроена HashMap, сложность основных операций? (Расскажите про принцип корзин)
...
## 94. Как устроена TreeMap, сложность основных операций?
...
## 95. Как, используя LinkedHashMap, сделать кэш c «invalidation policy»?
...
## 96. Какая коллекция реализует FIFO?
...
## 97. Какая коллекция реализует LIFO?
...
## 98. Какая коллекция реализует дисциплину обслуживания FIFO?
...
## 99. Какая коллекция реализует дисциплину обслуживания FILO?
...
## 100. Какая разница между fail-fast и fail-safe?
...
## 101. Какая разница между итераторами с fail-fast и fail-safe поведением? (С примерами)
...
## 102. Какие дополнительные методы для работы с ассоциативными массивами (maps) появились в Java 8?
...
## 103. Какие нововведения появились в java 8?
...
## 104. Какие промежуточные методы в стримах вы знаете?
...
## 105. Какие существуют реализации Map?
...
## 106. Какие существуют способы перебирать элементы списка?
...
## 107. Какие существуют способы создания стрима?
...
## 108. Каким образом можно получить синхронизированные объекты стандартных коллекций?
...
## 109. Какова оценка временной сложности операций над элементами из `HashMap`? Гарантирует ли `HashMap` указанную сложность выборки элемента?
...
## 110. Какова роль `equals()` и `hashCode()` в `HashMap`?
...
## 111. Каково максимальное число значений `hashCode()`?
...
## 112. Какое начальное количество корзин в `HashMap`?
...
## 113. Какое худшее время работы метода `add()` для `ArrayList`?
...
## 114. Какое худшее время работы метода `add()` для `LinkedList`?
...
## 115. Какое худшее время работы метода `contains()` для элемента, который есть в `ArrayList`?
...
## 116. Какое худшее время работы метода `contains()` для элемента, который есть в `LinkedList`?
...
## 117. Какое худшее время работы метода get(key) для ключа, которого нет в HashMap?
...
## 118. Кто кого расширяет: `Queue` расширяет `Deque`, или `Deque` расширяет `Queue`?
...
## 119. Можно ли итерируясь по ArrayList удалить элемент? Какое вылетит исключение?
...
## 120. Назовите главные реализации Deque
...
## 121. Назовите основные интерфейсы JCF и их реализации.
...
## 122. Напишите однопоточную программу, которая заставляет коллекцию выбросить `ConcurrentModificationException`.
...
## 123. Необходимо добавить 1 млн. элементов, какую структуру вы используете?
...
## 124. Объясните смысл параметров в конструкторе `HashMap(int initialCapacity, float loadFactor)`.
...
## 125. Почему `LinkedList` реализует и `List`, и `Deque`?
...
## 126. Почему несмотря на то, что ключ в `HashMap` не обязан реализовывать интерфейс `Comparable`, двусвязный список всегда удается преобразовать в красно-черное дерево?
...
## 127. Предложите эффективный алгоритм удаления нескольких рядом стоящих элементов из середины списка, реализуемого `ArrayList`.
...
## 128. Приведите пример реализации Deque.
...
## 129. Приведите пример, когда какая-либо коллекция выбрасывает `UnsupportedOperationException`.
...
## 130. Приведите примеры итераторов, реализующих поведение fail-safe
...
## 131. Расположите в виде иерархии следующие интерфейсы: `List`, `Set`, `Map`, `SortedSet`, `SortedMap`, `Collection`, `Iterable`, `Iterator`, `NavigableSet`, `NavigableMap`.
...
## 132. Расскажите о параллельной обработке в Java 8.
...
## 133. Расскажите про интерфейс List
...
## 134. Расскажите про интерфейс Set.
...
## 135. Расскажите про класс Collectors и его методы.
...
## 136. Расскажите про метод collect()
...
## 137. Расскажите про метод flatMap().
...
## 140. Расскажите про реализации интерфейса Set
...
## 141. Реализуйте симметрическую разность двух коллекций используя методы `Collection` (`addAll(...)`, `removeAll(...)`, `retainAll(...)`).
...
## 142. Сделайте `HashMap` из `HashSet<Map.Entry<K, V>>`.
...
## 143. Сделайте `HashSet` из ключей `HashMap`.
...
## 144. Сколько выделяется дополнительно памяти при вызове `LinkedList.add()`?
...
## 145. Сколько необходимо дополнительной памяти при вызове `ArrayList.add()`?
...
## 146. Сколько переходов происходит в момент вызова `HashMap.get(key)` по ключу, который есть в таблице?
...
## 147. Сколько создается новых объектов, когда вы добавляете новый элемент в `HashMap`?
...
## 148. Сколько элементов будет пропущено, если `Iterator.next()` будет вызван после 10-ти вызовов `Iterator.hasNext()`?
...
## 149. Согласно Кнуту и Кормену существует две основных реализации хэш-таблицы: на основе открытой адресации и на основе метода цепочек. Как реализована `HashMap`? Почему, по вашему мнению, была выбрана именно эта реализация? В чем плюсы и минусы каждого подхода?
...
## 150. Сравнение скорости работы ArrayList и LinkedList
...
## 151. Сравните `Iterator` и `ListIterator`.
...
## 152. Чем Set отличается от List?
...
## 153. Чем отличается `ArrayList` от `LinkedList`? В каких случаях лучше использовать первый, а в каких второй?
...
## 154. Чем отличаются методы map() и flatMap().
...
## 155. Что будет, если мы кладем в HashMap ключ, у которого equals и hashCode определены некорректно?
...
## 156. Что позволяет сделать `PriorityQueue`?
...
## 157. Что произойдет при вызове `Iterator.next()` без предварительного вызова `Iterator.hasNext()`?
...
## 158. Что работает быстрее `ArrayList` или `LinkedList`?
...
## 159. Что такое Deque? Чем отличается от Queue?
...
## 160. Что такое «fail-fast поведение»?
