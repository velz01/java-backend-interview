...
## 4. Шаблон «Уровень защиты от повреждений»
...
## 5. Шаблон «База данных на сервис» (Database Per Service)
...
## 7. Шаблон «Разделение команд и запросов» (Command Query Responsibility Segregation, CQRS)
...
## 8. Шаблон «Поиск событий» (Event Sourcing)
...
## 11. Шаблон «API-шлюз» (API Gateway)
...
## 12. Шаблон «Бэкенды для фронтендов» (Backends for Frontends, BFF)
...
## 13. Шаблон «Сборка пользовательского интерфейса на стороне клиента» (Client-Side UI Composition)
...
## 14. Шаблон «Сборка фрагментов страниц на стороне сервера» (Server-Side Page Fragment Composition)
...
## 15. Шаблон «Обнаружение сервисов на стороне клиента» (Client-Side Service Discovery)
...
## 16. Шаблон «Обнаружение сервисов на стороне сервера» (Server-Side Service Discovery)
...
## 17. Шаблон «Экземпляр сервиса на хост» (Service Instance Per Host)
...
## 18. Шаблон «Сине-зеленое развертывание» (Blue-Green Deployment)
...
## 19. Шаблон «Автоматический выключатель» (Circuit Breaker)
...
## 21. Шаблон «Агрегация логов» (Log Aggregation)
...
## 22. Шаблон «Распределенная трассировка» (Distributed Tracing)
...
## 23. Шаблон «Проверки здоровья» (Health Check)
...
## 24. Шаблон «Посредник» («Посол», Ambassador)
...
## 25. Шаблон «Коляска» («Прицеп», Sidecar)
...
## 26. Шаблон «Тестирование контрактов, ориентированных на потребителя» (Consumer-Driven Contract Testing)
