...
## 33. Что такое dependency management в Spring Boot?
...
## 34. Как правильно хранить пароль пользователя при использовании Spring Security?

Пароль не должен храниться в открытом виде и не должен шифроваться обратимым алгоритмом. Spring Security использует интерфейс `PasswordEncoder`; на практике пароль преобразуется односторонней медленной хэш-функцией с солью, например BCrypt. При входе приложение не расшифровывает хэш, а кодирует введённый пароль и проверяет соответствие через `PasswordEncoder.matches()`. Это снижает ущерб при утечке базы паролей.

---

## 35. Что такое Spring Profiles и как с ними работать?

Profiles позволяют включать разные наборы бинов и конфигурации для разных окружений, например `dev`, `test` и `prod`. Бин или конфигурационный класс можно ограничить аннотацией `@Profile("dev")`. Активные профили задаются через `spring.profiles.active`, переменные окружения, параметры запуска и другие источники конфигурации. Профили полезны, когда окружения действительно требуют разных бинов, но не должны превращаться в замену обычным параметрам конфигурации.

---

## 36. Какие виды proxy использует Spring?
...
## 37. Для чего нужна аннотация @Lazy?
...
## 38. Что делает @ControllerAdvice?

`@ControllerAdvice` позволяет вынести общую логику для нескольких MVC-контроллеров в отдельный компонент. Чаще всего вместе с `@ExceptionHandler` она используется для централизованного преобразования исключений в HTTP-ответы. Также через неё можно задавать общие `@ModelAttribute` и настройки binder. Для REST API часто применяют `@RestControllerAdvice`, которая дополнительно подразумевает сериализацию возвращаемых объектов в тело ответа.

---

## 39. Что такое @ConfigurationProperties и чем она отличается от @Value?
...
## 41. Что делает @Lookup?

`@Lookup` используется, когда singleton-бину при каждом вызове нужен новый экземпляр бина с более коротким scope, например prototype. Spring переопределяет такой метод в создаваемом прокси/подклассе и при каждом вызове получает объект из контейнера. Это решает проблему, когда обычное внедрение prototype в singleton дало бы только один экземпляр во время создания singleton.

---

## 42. Как работает BeanPostProcessor и зачем нужны before/after initialization?

`BeanPostProcessor` — расширение контейнера, которое может обработать каждый создаваемый бин до и после стадии инициализации. `postProcessBeforeInitialization()` вызывается перед lifecycle-методами вроде `@PostConstruct`, а `postProcessAfterInitialization()` — после них. На этом механизме Spring строит многие инфраструктурные возможности, в том числе создание прокси. Поэтому объект, который в итоге помещается в контекст, может отличаться от первоначально созданного экземпляра.

---

## 43. Как устроен Spring Security на уровне цепочки фильтров?

Защита веб-запросов строится вокруг цепочки servlet-фильтров. `DelegatingFilterProxy` передаёт работу Spring Security, а `FilterChainProxy` выбирает подходящую `SecurityFilterChain`. Отдельные фильтры выполняют аутентификацию, восстановление контекста безопасности, обработку исключений и авторизацию. Результат успешной аутентификации представлен объектом `Authentication`, который хранится в `SecurityContext` и доступен приложению через инфраструктуру `SecurityContextHolder`.

---

## 44. Что такое Spring MVC и какую задачу решает?
...
## 45. Чем @ResponseBody отличается от ResponseEntity?

`@ResponseBody` говорит Spring записать возвращаемое значение метода непосредственно в тело HTTP-ответа через подходящий `HttpMessageConverter`. `ResponseEntity` представляет весь ответ и дополнительно позволяет явно задать HTTP-статус и заголовки. Если нужен просто объект с обычным `200 OK`, достаточно `@ResponseBody`/`@RestController`; если нужно управлять статусом или headers, удобнее `ResponseEntity`.

---

## 46. Что делает @Conditional?

`@Conditional` позволяет регистрировать конфигурацию или бин только при выполнении заданного условия. Spring Boot активно развивает этот механизм через аннотации `@ConditionalOnClass`, `@ConditionalOnMissingBean`, `@ConditionalOnProperty` и другие. Благодаря условиям автоконфигурация создаёт инфраструктурные бины только тогда, когда они действительно нужны и пользователь не предоставил собственную реализацию.

---

## 47. В чём разница между @RequestParam и @PathVariable?
...
## 48. Как Spring обрабатывает исключения в MVC-приложении?

Исключение можно обработать локально методом контроллера с `@ExceptionHandler` или централизованно через класс `@ControllerAdvice`/`@RestControllerAdvice`. Spring MVC использует цепочку `HandlerExceptionResolver`, которая ищет подходящий обработчик и преобразует ошибку в HTTP-ответ. Такой подход позволяет отделить бизнес-логику от единого формата ошибок API.

---

## 49. Что делает @Configuration и чем она отличается от обычного @Component?
...
`@Async` позволяет выполнять вызов метода асинхронно через настроенный `TaskExecutor`. Как и многие другие возможности Spring, механизм обычно реализован через proxy, поэтому вызов асинхронного метода из того же объекта через `this` не проходит через перехватчик. Метод может возвращать `void`, `Future` или `CompletableFuture` в зависимости от сценария.

---

## 51. Для чего нужна @Scheduled?
...
Spring Data JPA строит слой репозиториев поверх JPA. Разработчик описывает интерфейс репозитория, а Spring создаёт реализацию автоматически. Запросы могут выводиться из имени метода, задаваться через `@Query`, спецификации и другие механизмы. Это уменьшает шаблонный код работы с `EntityManager`, но не отменяет необходимость понимать JPA, транзакции, fetch-стратегии и SQL, который в итоге выполняется.

---

## 53. Что такое Spring JDBC?

Spring JDBC упрощает прямую работу с JDBC, прежде всего через `JdbcTemplate`. Он берёт на себя типовой код открытия/закрытия ресурсов, обработку исключений и выполнение запросов, оставляя разработчику SQL и преобразование результатов. В отличие от ORM, здесь нет persistence context и автоматического отображения графа сущностей: разработчик значительно ближе к выполняемому SQL.
