...
## 26. Stream API & ForkJoinPool. Как связаны, что это такое.
...
## 28. Что такое Java Memory Model и happens-before?

Java Memory Model описывает правила видимости изменений памяти между потоками и допустимое переупорядочивание операций. Отношение happens-before гарантирует, что результат действий одного потока будет виден другому. Например, освобождение монитора happens-before последующему захвату того же монитора, запись в volatile-переменную happens-before последующему чтению этой же переменной, а действия в потоке до его запуска happen-before действиям запущенного потока. Отношение happens-before транзитивно.
